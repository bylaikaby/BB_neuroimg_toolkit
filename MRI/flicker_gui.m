...
% Default pattern: 8 s OFF, 4 s ON @ 2.5 Hz, 18 s OFF (Load Defaults / menu).
%
...
uimenu(mnu, 'Label', 'About', ...
    'Callback', @(~,~) uialert(fig, ...
        ['Flicker Stimulation GUI v1.0 - Direct NiDAQ control', newline, ...
         'Default pattern: 8 s OFF, 4 s ON @ 2.5 Hz, 18 s OFF.'], 'About', ...
        'Icon', 'info'));
...
stimGrid = uigridlayout(stimPanel, [8 2], ...
    'RowHeight', {26, 34, 34, 34, 34, 34, 34, 34}, ...
    'ColumnWidth', {135, '1x'}, ...
    'Padding', [12 10 12 10], ...
    'RowSpacing', 8, ...
    'ColumnSpacing', 10);

stimDesc = uilabel(stimGrid, ...
    'Text', 'Default pattern: 8 s OFF, 4 s ON, 18 s OFF @ 2.5 Hz', ...
    'FontAngle', 'italic', 'FontSize', 10, 'FontColor', [0.35 0.35 0.35]);
stimDesc.Layout.Row = 1;
stimDesc.Layout.Column = [1 2];
...
% ═══════════════════════════════════════════════════════════════════
    function writeLo(dq, p)
        if p.isDigital
            write(dq, false(1, numel(dq.Channels)));
        else
            write(dq, p.vLo);
        end
    end
    function writeHi(dq, p)
        if p.isDigital
            write(dq, true(1, numel(dq.Channels)));
        else
            write(dq, p.vHi);
        end
    end
