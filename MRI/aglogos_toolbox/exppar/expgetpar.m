...
%    2.10 26.08.25 YM  adapted for MonkeyLogic system at ICPBR.
%
...
if exist(expfilename(Ses,ExpNo,'bhv2'),'file')
  % ICPBR: MonkeyLogic system
  evt = expgetmlevt(Ses,ExpNo);
elseif exist(expfilename(Ses,ExpNo,'evt'),'file')
...
if isfield(grp,'daqver') && grp.daqver >= 3 && exist(expfilename(Ses,ExpNo,'bhv2'),'file')
  % ICPBR: MonkeyLogic system
  % stimulus parameters should be read from .stm file.
  stmpars = mlstm_read(expfilename(Ses,ExpNo,'stm'));
  stmpars.date = evt.date;
  pdmpars = {};
  hstpars = {};
elseif isfield(grp,'daqver') && grp.daqver >= 2 && exist(expfilename(Ses,ExpNo,'evt'),'file')
  % MPI: QNX-MriStim
...
elseif ~isfield(evt,'tfactor')
  ...
  elseif grp.daqver >= 3
    % ICPBR: MonkeyLogic
    if isrecording(grp)
      % MRI triggers are recorded in adfx.  Use this information/timings.
      adflen = evt.obs{1}.adflen;
      evtlen = evt.obs{1}.origtimes.end - evt.obs{1}.origtimes.begin;
      evt.tfactor = adflen / evtlen * evt.obs{1}.tfac_adf2mri;
    else
      % what to do...
      evt.tfactor = 1.0;
    end
...
if grp.daqver >= 3
  % ICPBR: MonkeyLogic system
  adflen = adf.obslen(1)*1000;  % in msec
  evtlen = evt.obs{1}.origtimes.end(1) - evt.obs{1}.origtimes.begin(1);
  evtlen = evtlen * evt.tfactor;
else
  % MPI: QNX-MriStim system
...
if grp.daqver >= 3
  ...
  for k=length(evt.obs):-1:1
    dgzlens(k) = evt.obs{k}.origtimes.end -  evt.obs{k}.origtimes.begin;
  end
else
  % MPI QNX-MriStim
  for k=length(evt.obs):-1:1
