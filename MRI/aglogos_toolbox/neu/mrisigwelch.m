function spc = mrisigwelch(SesName,ExpNo,ANAP)
%MRISIGWELCH - Compute PSD of the MRI signal using the Welch method
% MRISIGWELCH(Sig) uses Matlab's pwelch function to computer power spectral
...
Ses = goto(SesName);
Sig = sigload(SesName,ExpNo,'roiTs');
Sig = mroitsget(Sig,[],ANAP.roiname);
Sig = Sig{1};
Sig.dat = double(Sig.dat);
% We do this, because each session has different parameters...
% oSig = sigresample(Sig,NEWDX,varargin)
