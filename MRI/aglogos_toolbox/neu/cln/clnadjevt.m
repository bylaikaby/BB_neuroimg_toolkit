...
%    1.26 26.08.25 YM  adapted for ICPBR-MonkeyLogic system.
%
...
% if par.evt.nobsp>1
%     evt.system               = par.evt.system;
%     evt.systempar            = par.evt.systempar;
%     evt.dgzfile              = par.evt.dgzfile;
%     evt.physfile             = par.evt.physfile;
%     evt.nch                  = par.evt.nch;
%     evt.nobsp                = 1;
%     evt.dx                   = par.evt.dx;
%     evt.trigger              = par.evt.trigger;
%     evt.evttypes             = par.evt.evttypes(2:end);
%     evt.evtnames             = par.evt.evtnames(2:end);
%     evt.prmnames             = par.evt.prmnames;
%     evt.interVolumeTime      = par.evt.interVolumeTime;
%     evt.numTriggersPerVolume = par.evt.numTriggersPerVolume;
%     evt.obs{1}               = par.evt.obs{2};
%     evt.validobsp            = 1;
%     evt.tfactor              = 0.99774;
%     evt.date                 = par.evt.date;
% else
    evt = par.evt;
% end
...
for iObsp = 1:length(adfObslen),
  if grp.daqver >= 3
    % ICPBR-MonkeyLogic system
    evtlen = evt.obs{iObsp}.origtimes.end-evt.obs{iObsp}.origtimes.begin;
    tmpd = evtlen - adfObslen(iObsp)*adfDX;
    tmpd = tmpd / evtlen * 100; 
  else
    % MPI: QNX-MriStim system
  ...
  if grp.daqver < 3 && isempty(evt.obs{iObsp}.origtimes.mri),
...
if grp.daqver < 3 && pv.nseg*pv.nsli > 1,
  ...
  if any(strfind(lower(pv.acqp.PULPROG),'dualslice')) || any(strfind(lower(pv.acqp.PULPROG),'mbepi')),
      ...
      dt_trig     = pv.imgtr/pv.nseg/(pv.nsli/str2double(pv.method.NBands)) * 1000;  % in msec
	  pv.nsli = pv.nsli / str2double(pv.method.NBands); 
%       pv.gradtype = pv.gradtype(1:end/str2double(pv.method.NBands)); %db - in pv_getpvpars this is already handled now !!
...
if grp.daqver >= 3
  % ICPBR: MonkeyLogic system
  nmri_recorded = length(evt.obs{1}.mriIndx);
else
  % MPI: QNX-MriStim system
  for N = 1:length(evt.obs),
...
for iObsp = 1:NoObsp,
  ...
  if grp.daqver >= 3
    mriIndx = evt.obs{iObsp}.mriIndx;
    mriTime = mriIndx * adfDX * evt.obs{iObsp}.tfac_adf2mri;
  else
    % MPI: QNX-MriStim system
