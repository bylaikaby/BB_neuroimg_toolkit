...
if DEBUG > 0 || DO_PLOT > 0,
  ...
  if ~any(DO_CLEANING),
    ...
    if any(USE_REGRESS)
      tmpprm = sprintf('dbqNPCs=%d PrePCA=%.2fms PcaHP=%d',...
                       ANAP.NOPCS,ANAP.PREPCA*1000,ANAP.PCA_HIGHPASS);
    else
      tmpprm = sprintf('Lags=%d PCACoef=%.2f NPCs=%d NRem=%d PrePCA=%.2fms PcaHP=%d',...
                       ANAP.PCALAGS,ANAP.PCACOEF,ANAP.NOPCS,ANAP.NOREM,ANAP.PREPCA*1000,ANAP.PCA_HIGHPASS);
    end
  end
  MAX_TSEC = 0;
