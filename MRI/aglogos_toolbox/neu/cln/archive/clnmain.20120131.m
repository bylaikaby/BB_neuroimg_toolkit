...
%
% See also SESGETCLN, CLNADJEVT CLNADF
...
% PRINT INFO
% =================================================================
fprintf(' %s: %s ExpNo=%d  NObs=%d NChn=%d ',...
        mfilename,Ses.name,ExpNo,MriEvt.NoObsp,length(VALID_CHAN));
fprintf(' Dec=%d NPCs=%d NRem=%d PCACoef=%.2f PrePCA=%.1fms SaveGra=%d RemoveES=%d\n',...
        ANAP.DECFRAC,ANAP.NOPCS,ANAP.NOREM,ANAP.PCACOEF,ANAP.PREPCA*1000,...
        ANAP.SAVEGRA,ANAP.REMOVE_ES);
fprintf('          GradTypes=%d:[', length(MriEvt.gradtype));
  ...
  if 1,
    for iCh = 1:size(Cln.dat,2),
      ...
      if ~isempty(idx),
        ...
        fprintf(' %s: WARNING OUTLIERS(Ch=%d) %d/%d Values (p=%g)\n',...
