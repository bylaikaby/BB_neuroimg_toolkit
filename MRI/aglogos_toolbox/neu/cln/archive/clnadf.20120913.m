...
%      1. decimation, if required.
%      2. prepare PCA-data.
%      3. run PCA (for 'mean' subtracted PCA-data).
%      4. find PCs that correlates with 'mean' of PCA-data.
%      5. subtracts 'mean' and correlated PCs from PCA-data.
%      6. adjust gaps between edges of PCA-data, if required.
%
% VERSION :	
...
if isfield(ANAP,'PREPCA') && ~isempty(ANAP.PREPCA),
  PREPCA = round(ANAP.PREPCA/MriEvt.dx/ANAP.DECFRAC);
else
  ...
  %PREPCA = round(0.002/MriEvt.dx/ANAP.DECFRAC);		% 2ms
  PREPCA = round(0.0008/MriEvt.dx/ANAP.DECFRAC);		% 0.8ms
end
...
% now "mripits" must be relative to ADFOFFS
...
if ANAP.DECFRAC > 1,
  ...
  if length(ADFDAT)*8/1024/1024 > 320,
    % big vector, split into several periods.
    % note that filtfilt() will require 2 times more RAM in addition to ADFDAT.
    % so if ADFDAT is 320M, a total of 960M will be required.
    tmpn   = ceil(length(ADFDAT)*8/(320*1024*1024));
    tmpi   = round(length(ADFDAT)/tmpn/ANAP.DECFRAC)*ANAP.DECFRAC;
    tmpdat = {};
    for k = 1:tmpn,
      if k == tmpn,
        if length(tmpi*(k-1)+1:length(ADFDAT)) < 1000,
          tmpdat{k-1} = ADFDAT(tmpi*(k-2)+1:end);
        else
          tmpdat{k} = ADFDAT(tmpi*(k-1)+1:end);
        end
      else
        tmpdat{k} = ADFDAT([1:tmpi]+(k-1)*tmpi);
      end
    end
    ADFDAT = [];
    for k = 1:length(tmpdat),
      tmpdat{k} = decimate(tmpdat{k},ANAP.DECFRAC);
      ADFDAT = cat(1,ADFDAT(:),tmpdat{k});
      tmpdat{k} = [];
    end
    clear tmpdat tmpn tmpi k;
    if isfield(ANAP,'SAVEGRA') && ANAP.SAVEGRA > 0,
      % no need to do "CORRECT" decimation for gradient signal.
      % just pick up every "ANAP.DECFRAC" points.
      %dat1 = decimate(double(GRADAT(1:k)),ANAP.DECFRAC);
      %dat2 = decimate(double(GRADAT(k+1:end)),ANAP.DECFRAC);
      %clear GRADAT;
      %GRADAT = cat(1,dat(:),dat2(:));
      %clear dat1 dat2 k;
    ...
    if isfield(ANAP,'SAVEGRA') && ANAP.SAVEGRA > 0,
      %GRADAT = decimate(GRADAT, ANAP.DECFRAC);
      GRADAT = GRADAT(1:ANAP.DECFRAC:end);
    end
  end
  Sig.dx = MriEvt.dx * ANAP.DECFRAC;
  mripts = round(mripts/ANAP.DECFRAC);
  uniqlen = round(uniqlen/ANAP.DECFRAC);
end
...
if ~any(DO_CLEANING),
  ...
  for N = 1:length(MriEvt.gradtype),
    ...
    if 1,
      mNoise = mNoise / norm(mNoise);
      beta = mNoise' * PCADAT;
      for K = 1:size(PCADAT,2),
        PCADAT(:,K) = PCADAT(:,K) - beta(K)*mNoise;
      end
      %mNoise = mean(PCADAT,2);
    else
      for K = 1:size(PCADAT,2),
    ...
    if IS_MICROSTIM,
      ...
      if isempty(not_microstim),
        ...
        [PC, eVar, Proj, SigMean] = doPCA(dat,ANAP.NOPCS);
        dat = PCADAT';
        for K = 1:size(dat,2),
...
if isfield(ANAP,'SAVEGRA') && ANAP.SAVEGRA > 0,
  Sig.gra = int16(round(GRADAT(i0:end)));	% save as "int16", no need of "double".
end
...
if DEBUG > 0,
  ...
  MAX_TSEC = 0;
  if length(Sig.cln)*Sig.dx > 10*60,
    MAX_TSEC = 5*60;	% 5min max
  end
  ...
  plot((0:length(selidx)-1)*Sig.dx,ADFDAT(selidx),'color','red');
  hold on;
  % plot cleaned data
  ...
  subplot(2,2,3);
  NFFT = 2^nextpow2(0.2/Sig.dx);	% 0.2sec
  NOVERLAP = round(NFFT*0.2);
  Fs   = 1.0/Sig.dx;
  if isfield(ANAP,'SAVEGRA') && ANAP.SAVEGRA > 0,
    [Pxx,f] = pwelch(double(GRADAT),NFFT,NOVERLAP,NFFT,Fs);
    plot(f,Pxx,'green');  hold on;
  end
  [Pxx,f] = pwelch(ADFDAT,NFFT,NOVERLAP,NFFT,Fs);
  plot(f,Pxx,'red');  hold on;
  [Pxx,f] = pwelch(Sig.cln,NFFT,NOVERLAP,NFFT,Fs);
  plot(f,Pxx,'blue');
  grid on;
...
if DEBUG,
  i0 = round((MriEvt.SS_OFFS(ObspNo)-ADFOFFS)/ANAP.DECFRAC) + 1;
  ADF.dx = Sig.dx;
  ADF.dat = int16(round(ADFDAT(i0:end)));
...
for NPC = 1:size(PC,2),
  ...
  tmp = xcov(mNoise,PC(:,NPC),ANAP.PCALAGS,'coeff');
