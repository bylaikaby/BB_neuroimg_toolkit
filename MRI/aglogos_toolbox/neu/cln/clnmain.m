...
%     ANAP.clnpar.AVR_NOISE    = 'mean';    % averaged noise to subtract, 'mean' | 'median'
...
%     ANAP.clnpar.REMOVE_ECG   = 0;         % try to remove heart-beat artifact
...
%   1.16 10.07.14 YM  supports "AVR_NOISE".
%
...
ANAP.PLOT        = ANAP.DEBUG;  % Plot data before/after cleaning
ANAP.UPSAMPLE    = 0;
...
ANAP.AVR_NOISE   = 'mean';      % averaged noise to subtract, 'mean' | 'median'
...
ANAP.REMOVE_ECG  = 0;           % try to remove heart-beat artifact
...
if sesversion(Ses) >= 2,
  ...
  if ~exist(fname,'file'),
    ...
    clnadjevt(Ses,ExpNo,'interactive',0,'upsample',ANAP.UPSAMPLE);
    fprintf(' done.\n');
    ...
    MriEvt = load(fname,'clnadj');
    MriEvt = MriEvt.clnadj;
    if any(ANAP.UPSAMPLE) && ~isfield(MriEvt,'upsample')
      fprintf(' %s: no upsampled events, doing clnadjevt()...',mfilename);
      clnadjevt(Ses,ExpNo,'interactive',0,'upsample',ANAP.UPSAMPLE);
      fprintf(' done.\n');
      MriEvt = load(fname,'clnadj');
      MriEvt = MriEvt.clnadj;
  ...
  if ~exist('ClnAdjEvt.mat','file') || isempty(who('-file','ClnAdjEvt.mat',MriEvtName)),
    fprintf(' %s: no MRI event data, doing clnadjevt()...',mfilename);
    clnadjevt(Ses,ExpNo,'interactive',0);
    fprintf(' done.\n');
  end
...
if any(ANAP.DEBUG),  ANAP.PLOT = 1;  end
% =================================================================
% PRINT INFO
% =================================================================
fprintf(' %s: %s Exp=%d  NObs=%d NCh=%d ',...
        mfilename,Ses.name,ExpNo,MriEvt.NoObsp,length(VALID_CHAN));
fprintf(' Upsample=%d Dec=%d SaveGra=%d RemoveES/ECG=%d/%d AVR_NOISE=%s  DEBUG/PLOT=%d/%d\n',...
        ANAP.UPSAMPLE,ANAP.DECFRAC,ANAP.SAVEGRA,ANAP.REMOVE_ES,ANAP.REMOVE_ECG,ANAP.AVR_NOISE,ANAP.DEBUG,ANAP.PLOT);
switch lower(ANAP.METHOD)
 case {'pca'}
  fprintf('     PCA: NPCs=%d NRem=%g PCACoef=%g PrePCA=%.1fms PcaHP=%d UseLANSVD=%d ScaleSubtract=%d',...
  ...
  if 1,
    for iCh = 1:size(Cln.dat,2),
      ...
      if ~isempty(idx),
        ...
        fprintf(' %s: WARNING OUTLIERS(Ch=%2d) %7d/%d points (p=%g)\n',...
