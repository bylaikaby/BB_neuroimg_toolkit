...
%
% NOTES :
%    * PCA_HIGHPASS improves cleaning when PCACoef is low (~0.10).
%    * METHOD='regress' takes 30% more time than 'pca'.
%
% VERSION :	
...
%   1.16 06.11.13 YM  use svds* without a covariance matrix, less memory and 20% faster.
%   1.17 10.06.14 YM  scale the spectrum (spectrogram) by NFFT.
%   1.18 10.06.14 YM  supports method (regress), PCA_HIGHPASS.
%   1.20 26.06.14 YM  experimental support of upsampling (no improvement observed..).
%   1.21 08.07.14 YM  bug fix when plotting in the "no-cleaning" mode.
%   1.22 10.07.14 YM  supports "AVR_NOISE".
%   1.23 29.10.14 YM  more supports for microstimulation.
%   1.24 26.09.18 YM  ignores periods with ES for plotting spectrums.
%
...
if isfield(ANAP,'PLOT') && ~isempty(ANAP.PLOT)
  DO_PLOT = ANAP.PLOT;
else
  DO_PLOT = DEBUG;
end

if ~isfield(ANAP,'AVR_NOISE') || isempty(ANAP.AVR_NOISE)
  ANAP.AVR_NOISE = 'mean';
end
...
if ~isfield(ANAP,'PREPCA') || isempty(ANAP.PREPCA)
  ...
  %ANAP.PREPCA = 0.002;		% 2.0ms
  ANAP.PREPCA = 0.0008;		% 0.8ms
end
...
UPSAMPLE_FAC = 0;
if isfield(ANAP,'UPSAMPLE') && any(ANAP.UPSAMPLE)
  UPSAMPLE_FAC = MriEvt.upsample.factor;
end
if ~any(DO_CLEANING),  UPSAMPLE_FAC = 0;  end
...
%ADFOFFS = min(mripts);
ADFOFFS = mripts(1) - round(0.01/MriEvt.dx);
...
% update some parameters for upsampled data.
if UPSAMPLE_FAC > 1
  mripts_ds = mripts;  % keep this for plotting
  mripts = MriEvt.upsample.SS_PNTS(MriEvt.SS_OBSP == ObspNo);
  mripts = mripts - ADFOFFS*UPSAMPLE_FAC + 1;	% +1 for matlab indexing
  % update some parameters
  uniqlen = uniqlen*UPSAMPLE_FAC;
  PREPCA  = PREPCA*UPSAMPLE_FAC;
end
...
IS_MICROSTIM = 0;
if any(subIsStimType(par.stm.stmtypes,'microstim'))
  IS_MICROSTIM = 1;
  ...
  % first chunks of each volume
  firstchunks = mripts(1:length(MriEvt.gradtype):end);
  stmv = par.stm.v{ObspNo};
  stmt = par.stm.tvol{ObspNo} + 1;  % +1 for matlab indexing
  for N = 1:length(stmv)
    if any(subIsStimType(par.stm.stmtypes(stmv(N)+1),'microstim'))
      ts = firstchunks(stmt(N));
      te = firstchunks(stmt(N+1))-1;
      microstim_v(ts:te) = 1;
    end
  end
  clear stmv stmt firstchunks;
end
...
if isfield(ANAP,'REMOVE_ES') && ANAP.REMOVE_ES > 0
  if ANAP.REMOVE_ES > 10000
    ADFDAT = subRemoveES(ADFDAT,MriEvt.dx,ANAP.REMOVE_ES);
  else
...
if any(DO_CLEANING)
  ...
  % ANAP.LOWPASS = 4500;
  % ADFDAT = upsample(ADFDAT,4);
  % fprintf(' LP[%gkHz]',ANAP.LOWPASS/1000);
  % clear tmpsig;
  % tmpsig.dx = Sig.dx/4;
  % tmpsig.dat = ADFDAT(:);
  % tmpsig = sigfiltfilt(tmpsig,ANAP.LOWPASS,'low');
  % ADFDAT = tmpsig.dat(:);
  % clear tmpsig;
  % ADFDAT = decimate(ADFDAT,4);
...
if ~any(DO_CLEANING)
  ...
  if UPSAMPLE_FAC > 1
    fprintf(' upsample[%.1fkHz]',(1/MriEvt.dx)*UPSAMPLE_FAC/1000);
    xi = (1:length(ADFDAT)*UPSAMPLE_FAC)-1;
    x  = xi(1:UPSAMPLE_FAC:end);
    ADFDAT = interp1(x,ADFDAT,xi,'spline');
    ADFDAT = ADFDAT(:);
    clear x xi;
  end
  
  % DO CLEANING
  fprintf(' clean[%s] ',ANAP.METHOD);
  USE_REGRESS = any(strcmpi(ANAP.METHOD,'regress'));
  ...
  for N = 1:length(uniqidx)
    ...
    % Subtract the mean as a major noise component.
    if any(strcmpi(ANAP.AVR_NOISE,'median'))
      if isempty(not_microstim)
        mNoise = median(PCADAT,2);
      else
        mNoise = median(PCADAT(:,not_microstim),2);
      end
    else
      if isempty(not_microstim)
        mNoise = mean(PCADAT,2);
      else
        mNoise = mean(PCADAT(:,not_microstim),2);
      end
    end
    if any(USE_REGRESS)
      % CLEAN BY REGRESS
      ...
      % CLEAN BY PCA
      if isfield(ANAP,'SCALE_SUBTRACT') && ANAP.SCALE_SUBTRACT > 0
        % SCALED mean-subtraction:
        % this helps for unstable noises but not much improvement for stable ones.
        mNoiseN = mNoise / norm(mNoise);
        beta = mNoiseN' * PCADAT;
        % X = [];
        % X(:,1) = mNoiseN;
        % X(:,2) = 1;
        % beta2 = zeros(1,size(PCADAT,2));
        % for K = 1:size(PCADAT,2)
        %   tmpb = regress(PCADAT(:,K),X);
        %   beta(K) = tmpb(1);
        % end
        % clear X;
    ...
    if isempty(not_microstim)
      [PC, eVar, Proj, SigMean] = doPCA(PCADAT,ANAP.NOPCS,ANAP.USE_LANSVD,ANAP.PCA_HIGHPASS);
    else
      % this may improve artifact caused by microstimulation
      dat = PCADAT(:,not_microstim);
      [PC, eVar, Proj, SigMean] = doPCA(dat,ANAP.NOPCS,ANAP.USE_LANSVD,ANAP.PCA_HIGHPASS);
      dat = PCADAT';
      for K = 1:size(dat,2)
    ...
    if any(USE_REGRESS)
      % CLEAN BY REGRESS
      [RECODAT,pcidx,pcacoef] = subREGRESS(PCADAT,ANAP,PC,eVar,Proj,SigMean,mNoise,ChanNo,MriEvt,ObspNo);
      fprintf('%d',length(pcidx));
    else
      % CLEAN BY PCA
      [RECODAT,pcidx,pcacoef] = subPcaRECO(PCADAT,ANAP,PC,eVar,Proj,SigMean,mNoise,ChanNo,MriEvt,ObspNo);
      fprintf('%d',length(pcidx));
    end
    % figure('Name',sprintf('chunk%2d',N));
    % plot(mean(RECODAT,2)); set(gca,'ylim',[-10 10]);
    % if ~isempty(not_microstim),
    %   hold on;
    %   plot(mean(RECODAT(:,not_microstim),2),'g');
    %   length(not_microstim)
    %   keyboard
    % end
    % tmpreco1 = RECODAT(100:400,:);
    % tmpreco2 = RECODAT(500:800,:);
    % fprintf('\n%2d: NoiseSD=%.2f SigSD=%g-%g/%g',N,std(mNoise),std(tmpreco1(:)),std(tmpreco2(:)),std(RECODAT(:)));
    % if N==2 || N==10
    %   keyboard
    % end
    ...
    if DEBUG > 0
      MatFile = sprintf('%s/%s_ob%03d_ai%03d_pca_grad%d.mat',...
  ...
  if UPSAMPLE_FAC > 1
    fprintf(' Downsample');
    Sig.cln = Sig.cln(1:UPSAMPLE_FAC:end);
    %Sig.cln = decimate(Sig.cln,UPSAMPLE_FAC);
    if ~isempty(ADFLOW) && length(Sig.cln) > length(ADFLOW)
      Sig.cln = Sig.cln(1:length(ADFLOW));
    end
    if DEBUG > 0 || DO_PLOT > 0
      % need for plotting...
      %ADFDAT = decimate(ADFDAT,UPSAMPLE_FAC);
      ADFDAT = ADFDAT(1:UPSAMPLE_FAC:end);
      if ~isempty(ADFLOW) && length(ADFDAT) > length(ADFLOW)
        ADFDAT = ADFDAT(1:length(ADFLOW));
      end
      mripts = mripts_ds;
    end
  end
...
if ~isempty(ADFLOW)
  ...
  if DEBUG > 0 || DO_PLOT > 0,  ADFDAT  = ADFDAT  + ADFLOW;  end  % need for plotting
end
clear ADFLOW;
...
if DEBUG > 0 || DO_PLOT > 0
  ...
  tmptxt = sprintf('%s [%s]: %s Exp:%d AiCh:%d',mfilename,ANAP.METHOD,Ses.name,MriEvt.ExpNo,ChanNo);
  if ~any(DO_CLEANING)
    tmpprm = 'no-cleaning';
  else
    if any(USE_REGRESS)
      tmpprm = sprintf('AvrNoise=%s dbqNPCs=%d PrePCA=%.2fms PcaHP=%d',...
                       ANAP.AVR_NOISE,ANAP.NOPCS,ANAP.PREPCA*1000,ANAP.PCA_HIGHPASS);
    else
      tmpprm = sprintf('AvrNoise=%s Lags=%d PCACoef=%.2f NPCs=%d NRem=%d PrePCA=%.2fms PcaHP=%d',...
                       ANAP.AVR_NOISE,ANAP.PCALAGS,ANAP.PCACOEF,ANAP.NOPCS,ANAP.NOREM,ANAP.PREPCA*1000,ANAP.PCA_HIGHPASS);
    end
  end
  MAX_TSEC = 0;
  ...
  if IS_MICROSTIM
    tmpes = microstim_v;
    if length(tmpes) > length(ADFDAT)
      tmpes = tmpes(1:length(ADFDAT));
    end
    [Pxx,f] = pwelch(ADFDAT(tmpes == 0),NFFT,NOVERLAP,NFFT,Fs);
    clear tmpes;
  else
  ...
  if IS_MICROSTIM
    if ANAP.DECFRAC > 1
      tmpes = int8(round(sub_decimate(double(microstim_v), ANAP.DECFRAC)));
    else
      tmpes = microstim_v;
    end
    if length(tmpes) > length(Sig.cln)
      tmpes = tmpes(1:length(Sig.cln));
    end
    tmpdat = Sig.cln(tmpes == 0);
    [Pxx,f] = pwelch(tmpdat-mean(tmpdat(:)),NFFT,NOVERLAP,NFFT,Fs);
    clear tmpdat tmpes;
  else
  ...
  %   plot(F,mean(abs(S),2)/NFFT,'green');  hold on;
  % end
  % DX = MriEvt.dx;
  % NFFT = 2^nextpow2(TWIN/DX); NOVERLAP = round(NFFT*0.2);  Fs   = 1.0/DX;
  % [S,F,T] = spectrogram(ADFDAT,NFFT,NOVERLAP,NFFT,Fs);
  % plot(F,mean(abs(S),2)/NFFT,'red');  hold on;
  % DX = Sig.dx;
  % NFFT = 2^nextpow2(TWIN/DX); NOVERLAP = round(NFFT*0.2);  Fs   = 1.0/DX;
  % [S,F,T] = spectrogram(Sig.cln,NFFT,NOVERLAP,NFFT,Fs);
  % plot(F,mean(abs(S),2)/NFFT,'blue');
  % grid on;
  ...
  mmkdir(fp);
  figfile = fullfile(fp,sprintf('figs/%s_ai%03d.fig',fr,ChanNo));
  mmkdir(fileparts(figfile));
  saveas(gcf,figfile);
  close(gcf);
end
...
MATFILE = sprintf('%s_ob%03d_ai%03d.mat',...
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [PC, eVar, Proj, SigMean] = doPCA(dat, nopcs, USE_LANSVD, PCA_HIGHPASS)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% this is 20% faster than doPCA_OLD()

% dat as (time,repeats)
SigMean	= mean(dat,2);					% mean value along repeats
for N = 1:size(dat,1)
  dat(N,:) = dat(N,:) - SigMean(N);		% center the data
end

if any(PCA_HIGHPASS)
  tmpsig.dx = 1;  % normalized freq.
  tmpsig.dat = dat;
  tmpsig = sigdetrend(tmpsig);
  tmpsig = sigfiltfilt(tmpsig,1/size(tmpsig.dat,1)*3,'high');
  dat = tmpsig.dat;
  clear tmpsig;
end

  
% [U,S,V] = SVDS(dat,nopcs) computes the the nopcs first singular
...
if any(USE_LANSVD)
  ...
  [u,s,v] = svds_lansvd(dat, nopcs, 'L');
else
  [u,s,v] = svds(dat, nopcs);	% find singular values
end
%save('d:/temp/pcanew.mat','u','s','v');

PC    = u;
eVar  = diag(s);						% turn diagonal mat into vector.
SigMean = SigMean(:);					% return mean
Proj  = v*s;							% Proj centered dat onto PCs.

return;

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [PC, eVar, Proj, SigMean] = doPCA_OLD(dat, nopcs, USE_LANSVD)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% NOTE:  [u s v] = svds(A,...)  
% when 'A' is a covariance matrix, 'u' and 'v' is essentially the same (u'*v=eye).
...
%save('d:/temp/pcaold.mat','U','eVar','PC');
clear tmpcov;
...
% =======================================================================
function [RECODAT, pcidx, pcacoef] = subPcaRECO(PCADAT,ANAP,PC,eVar,Proj,SigMean,mNoise,ChanNo,MriEvt,ObspNo,CHECK)
% =======================================================================

if ~exist('CHECK','var'),  CHECK = 1;  end

% FracVar = eVar / sum(eVar);
FracVar = eVar / sum(std(PCADAT,[],2));
...
elseif ANAP.NOREM < 1
  pcidx = find(FracVar(:) > ANAP.NOREM & pcacoef(:) > ANAP.PCACOEF);
else
...
if CHECK == 1 && isempty(pcidx)
  fprintf(' %s[WARNING]: Exp=%d, Obsp=%d, AiCh=%d  ',...
          mfilename,MriEvt.ExpNo, ObspNo, ChanNo);
  fprintf('No PC was found that is significantly correlated with mean interference.\n');
  if ANAP.NOREM <= 0
    fprintf(' PCs FracVar  (thr=%g): %s\n',0.02,deblank(sprintf('%.3f ',FracVar)));
  elseif ANAP.NOREM < 1
    fprintf(' PCs FracVar  (thr=%g): %s\n',ANAP.NOREM,deblank(sprintf('%.3f ',FracVar)));
  else
    fprintf(' PCs FracVar  (nrem=%d): %s\n',ANAP.NOREM,deblank(sprintf('%.3f ',FracVar)));
  end
  fprintf(' PCs CorrCoef (thr=%g): %s\n',ANAP.PCACOEF,deblank(sprintf('%.3f ',pcacoef)));
end
...
% =======================================================================
function [RECODAT,pcidx,pcacoef] = subREGRESS(PCADAT,ANAP,PC,eVar,Proj,SigMean,mNoise,ChanNo,MriEvt,Obs)
% =======================================================================

% now checks peaks...
tmpnoise = abs(zscore(mNoise));
if max(tmpnoise) < 1.5
  % ok, no sharp peaks in the mean noise.  run normal PCA-based stuff...
  [RECODAT,pcidx,pcacoef] = subPcaRECO(PCADAT,ANAP,PC,eVar,Proj,SigMean,mNoise,ChanNo,MriEvt,ObspNo);
  return;
end

% yes, run regression-based cleaning...
[tmpv, locs] = findpeaks(tmpnoise,'MINPEAKHEIGHT',1.5);
noisepks = zeros(size(PC,1),1);
tmpwin   = -round(0.00015/MriEvt.dx):round(0.00015/MriEvt.dx);
for N = 1:length(locs)
  tmpidx = tmpwin + locs(N);
  tmpidx = tmpidx(tmpidx > 0 & tmpidx < length(noisepks));
  noisepks(tmpidx) = 1;
end
% picks up "noise" related components, peaks only. (ignore any slow components)
tmppc.dx = MriEvt.dx;
tmppc.dat = PC;
if ~any(ANAP.PCA_HIGHPASS)
  % if not PCA is not high-passed, then apply highpass to focus on peakes.
  tmppc = sigdetrend(tmppc);
  tmppc = sigfiltfilt(tmppc,1/(size(tmppc.dat,1)*tmppc.dx)*3,'high');
end
tmpwin   = -round(0.0002/MriEvt.dx):round(0.0002/MriEvt.dx);
pcidx = [];
for N = 1:size(PC,2)
  tmpdat = abs(zscore(tmppc.dat(:,N)));
  if max(tmpdat) < 3.5,  continue;  end
  [tmpv, tmplocs] = findpeaks(tmpdat,'MINPEAKHEIGHT',3.5);
  if any(noisepks(tmplocs) > 0)
    pcidx(end+1) = N;
    tmppks = zeros(size(noisepks));
    for K = 1:length(tmplocs)
      tmpidx = tmpwin + tmplocs(K);
      tmpidx = tmpidx(tmpidx > 0 & tmpidx < length(noisepks));
      tmppks(tmpidx) = 1;
    end
    % ignore small changes...
    tmppc.dat(tmppks==0,N) = 0;
  end
end

% make a matrix of regressors
X = [];
X = cat(2,SigMean(:)/norm(SigMean),tmppc.dat(:,pcidx));
NDer = 2;  % max order of the derivatives
tmpdiff = X;
for K = 1:NDer
  tmpdiff = diff([tmpdiff(1,:); tmpdiff],1,1);
  X = cat(2,X,tmpdiff);
end
for K = 1:size(X,2)
  X(:,K) = X(:,K) / norm(X(:,K));
...
RECODAT = PCADAT;
% for K = 1:size(RECODAT,2)
%   b = glmfit(X,PCADAT(:,K),'normal','link','identity');
%   RECODAT(:,K) = glmval(b,X,'identity');
% end
X(:,end) = 1;
BETA = zeros(size(X,2),size(RECODAT,2));
for K = 1:size(RECODAT,2)
  tmpb = regress(PCADAT(:,K),X);
  tmpb(end) = 0;  % do not subtract the DC offset...
  RECODAT(:,K) = PCADAT(:,K) - X*tmpb;
  BETA(:,K) = tmpb(:);
end
pcacoef = [];
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% subfunction to detect the stimulus type
function VAL = subIsStimType(StimType,StimStr)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if iscell(StimType)
  VAL = zeros(size(StimType));
  for N = 1:numel(StimType)
    VAL(N) = subIsStimType(StimType{N},StimStr);
  end
  VAL = reshape(VAL,size(StimType));
  return
end

VAL = any(strfind(StimType,StimStr));
