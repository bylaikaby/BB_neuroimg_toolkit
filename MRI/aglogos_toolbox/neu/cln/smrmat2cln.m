...
%    1.00 10.11.15 RN/YM  supports direct reading of SMR.
%    1.10 18.11.15 YM  supports reading by MEX without SON2.
%    1.11 xx.zz.15 RMN supports artifact removal.
%
%  See also expgetpar_spike2 sesgetcln expfilename smrmat2spk smr_read
...
if ~exist(SMRFILE,'file')
  error(' ERROR %s(%s,Exp=%d): file not found, %s\n',mfilename,Ses.name,ExpNo,SMRFILE);
end
...
CLNDAT = [];  CLNDX = NaN;  SMRHEADER = [];  SMRCHLIST = [];

for K = 1:length(SPIKE2.data),
  if any(exist('pdot.m','file')
    pdot(K,length(SPIKE2.data),'Reading channels:')    
  else
    fprintf('.');
  end
  ...
  if strcmpi(SMRFILE(end-3:end),'.smr')
    % load the smr file
    if any(SIGTOOL_LIB) && isempty(SMRHEADER),
    ...
    chnum = smr_findchan(SMRFILE,SPIKE2.data{K},'channellist',SMRCHLIST,'son2',SIGTOOL_LIB);
    if ~any(chnum)
      error(' ERROR %s: chan(''%s'') not found in %s\n',mfilename,SPIKE2.data{K},SMRFILE);
    end
    if ~isfield(Ses.expp(ExpNo),'smrwin'),  Ses.expp(ExpNo).smrwin = [];  end
    tmpdata = smr_read(SMRFILE,chnum,'channellist',SMRCHLIST,...
                       'window',Ses.expp(ExpNo).smrwin,'son2',SIGTOOL_LIB);
    if isempty(tmpdata) || ~isfield(tmpdata,'values'),
      error(' ERROR %s: invalid data channel ''%s''.\n',mfilename,SPIKE2.data{K});
    end
  else
