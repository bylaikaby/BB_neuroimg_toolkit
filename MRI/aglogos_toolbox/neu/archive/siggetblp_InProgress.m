...
% YM  13.02.06  do MUA bands first to avoid memroy problem of d01nm5,exp=9
%
% FINAL (I hope!)
% info.band{ 1}       = {[   0   3.5] 'Delta'   'LFP', 0};
% info.band{ 2}       = {[ 3.5   7.5] 'Theta'   'LFP', 0};
% info.band{ 3}       = {[ 8.5  13.5] 'Alpha'   'LFP', 3};
% info.band{ 4}       = {[14.5  23.5] 'Beta'    'LFP', 4};
% info.band{ 5}       = {[24.5    90] 'Gamma'   'LFP', 30};
% info.band{ 6}       = {[  90   130] 'Zeta'    'LFP', 30};
% info.band{ 7}       = {[   0    90] 'LFP'     'LFP', 0};
% info.band{ 8}       = {[  40   130] 'LFPN'    'LFP', 45};
% info.band{ 9}       = {[ 400  3000] 'MUA'     'MUA', 45};
%  
% 1. What is the most "meaninful" way of using the LFP-bands as representatives of different
% cellular events. That, is what is the best way of using this signal, so that any
% correlations between them arise from pure biological phenomena and not the filtering and
% rectification procedure?
%
% 2. Provided we have the "best bands". Then, we take the MUA envelop and we filter it with
% identical cutoffs (band pass for most signals) to those used for the LFP-bands. So, we
% have for eacn LFP (e.g. 4-8Hz) a rectified MUA (rMUA) of 4-8Hz.
%
% 3. We then ask the question. Can we use the LFP bands to predict the MUA envelope?
%    MUA (1:N) = B LFP-Bands
%
% TO check some of the ideas we discussed, DO NOT rectify ANYTHING
% Extract the bands for the best sessions (c98 etc.) and dump them.
% THen we try (a) normal rectification, (b) rectification of the diff, (c) diff of the
% rectified signals,  and then apply  MultReg etc.
...
info.band{ 1}       = {[   0   3.5] 'Delta'   'LFP', 0};
info.band{ 2}       = {[ 3.5   7.5] 'Theta'   'LFP', 0};
info.band{ 3}       = {[ 3.5   7.5] 'ThetaR'  'LFP', 2};
info.band{ 4}       = {[ 8.5  13.5] 'Alpha'   'LFP', 3};
info.band{ 5}       = {[14.5  23.5] 'Beta'    'LFP', 4};
info.band{ 6}       = {[24.5    90] 'Gamma'   'LFP', 30};
info.band{ 7}       = {[   0    90] 'LFP'     'LFP', 0};
info.band{ 8}       = {[  90   130] 'LFPH'    'LFP', 30};
info.band{ 9}       = {[  40   130] 'LFPN'    'LFP', 45};
info.band{10}       = {[ 400  3000] 'MUA'     'MUA', 45};
...
for B=info.lBands,
  band = info.band{B};
  % band pass filtering %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  fprintf('BP[%.1f-%.1f].',band{1}(1),band{1}(2));
  if isempty(SAME_SIG),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [b,a,fname,ftype,forder] = subDesignPMFilter(Fs,lim,mode,edgeFlt,info)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  fsamp = 1/Sig.dx;
if lim(1) ==  0, % for the Delta range
  mode = 'low';
...
switch lower(mode),
 case {'lowpass','lp','low'}
  ...
  mags = [1 0];
  devs = [abs(1-10^(info.passripple/20)) 10^(-info.dB/20)];
  [n,fo,ao,w] = firpmord(fcuts,mags,devs,Fs);
  n = n + rem(n,2);
  b = firpm(n,fo,ao,w);
  a = 1;
 case {'highpass','hp','high'}
...
fname  = 'fir1-firpm';
ftype  = 'firpm'; % given by above kaiserord().
forder = n;
