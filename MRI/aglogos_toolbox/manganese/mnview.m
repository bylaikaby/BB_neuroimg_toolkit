...
%    1.03 17.09.10 YM   supports mask by file
%    1.04 06.02.12 YM   use mroi_file().
%
...
% load anatomy data
if sesversion(Ses) >= 2,
  ANA = sigload(Ses,grp.ana{2},grp.ana{1});
else
...
StatmapTxt = uicontrol(...
    ...
    'Units','char','Position',[XDSP-5 H-0.3 30 1.5],...
...
StatmapEdt = uicontrol(...
    ...
    'Units','char','Position',[XDSP+3 H 107 1.5],...
...
StatmapCheck = uicontrol(...
    ...
    'Units','char','Position',[XDSP+112 H 20 1.5],...
...
% WIDGETS TO SELECT STATISTICAL MAP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
XDSP = 10; H = figH - 4.5;
MaskmapTxt = uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP-5 H-0.3 30 1.5],...
    'String','MASK:','FontWeight','bold',...
    'HorizontalAlignment','left',...
    'Tag','MaskmapTxt',...
    'BackgroundColor',get(hMain,'Color'));
MaskmapEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+3 H 90 1.5],...
    'Callback','mnview(''Main_Callback'',gcbo,''edit-maskmap'',guidata(gcbo))',...
    'String','','Tag','MaskmapEdt',...
    'HorizontalAlignment','left',...
    'TooltipString','mask map file',...
    'FontWeight','Bold');
MaskmapReadBtn = uicontrol(...
    'Parent',hMain,'Style','PushButton',...
    'Units','char','Position',[XDSP+94 H 16 1.5],...
    'Callback','mnview(''Main_Callback'',gcbo,''browse-maskmap'',guidata(gcbo))',...
    'Tag','MaskmapReadBtn','String','Browse...',...
    'TooltipString','browse a mapfile','FontWeight','Bold');
MaskmapCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+112 H 20 1.5],...
    'Tag','MaskmapCheck','Value',1,...
    'Callback','mnview(''Main_Callback'',gcbo,''redraw'',guidata(gcbo))',...
    'String','Apply','FontWeight','bold',...
    'TooltipString','Mask by file','BackgroundColor',get(hMain,'Color'));
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+124 H-0.3 50 1.5],...
    'String','MaskRange:','FontWeight','bold',...
    'HorizontalAlignment','left',...
    'Tag','MaskMinMaxTxt',...
    'BackgroundColor',get(hMain,'Color'));
MaskmapMinMaxEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+138 H 21 1.5],...
    'Callback','mnview(''Main_Callback'',gcbo,''update-maskmap'',guidata(gcbo))',...
    'String','','Tag','MaskmapMinMaxEdt',...
    'HorizontalAlignment','center',...
    'TooltipString','set the valid range [min max]',...
    'FontWeight','bold');
...
AlphaEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+3 H 15 1.5],...
...
MaskBlackCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+65 H 30 1.5],...
    'Tag','MaskBlackCheck','Value',0,...
    'Callback','mnview(''Main_Callback'',gcbo,''redraw'',guidata(gcbo))',...
    'String','Hide Low-Intensity','FontWeight','bold',...
    'TooltipString','Hide black regrions','BackgroundColor',get(hMain,'Color'));
...
switch lower(eventdata),
 ...
 case {'save-statmap'}
  STATMAP = getappdata(wgts.main,'STATMAP');
  MASKMAP = getappdata(wgts.main,'MASKMAP');
  if ~isempty(STATMAP),
    mapfile = get(wgts.StatmapEdt,'String');
    ...
    if ~isequal(mapfile,0) && ~isequal(pathname,0),
      ...
      set(wgts.StatmapEdt,'String',fullpathname);
      eval('STATS = STATMAP;');
      if get(wgts.MaskmapCheck,'Value') > 0 && ~isempty(MASKMAP) && isfield(STATS,'mask')
        [fp fr fe] = fileparts(get(wgts.MaskmapEdt,'String'));
        STATS.mask.maskmap = sprintf('%s%s',fr,fe);
        STATS.mask.maskmap_nomask  = str2num(get(wgts.MaskmapMinMaxEdt,'String'));
        STATS.mask.dat(MASKMAP.dat(:) == 0) = 0;
      end
      ANAP = getappdata(wgts.main,'ANAP');
      permute_vec = ANAP.mnview.permute;
      if ~isempty(permute_vec),
 ...
 case {'browse-maskmap'}
  ...
  if ~isequal(mapfile,0) && ~isequal(pathname,0),
    fullpathname = fullfile(pathname,mapfile);
    set(wgts.MaskmapEdt,'String',fullpathname);
    Main_Callback(hObject,'edit-maskmap',[]);
  end
 
 case {'edit-maskmap' 'update-maskmap'}
  mapfile = get(wgts.MaskmapEdt,'String');
  if ~exist(mapfile,'file'),
    setappdata(wgts.main,'MASKMAP',[]);
    %Main_Callback(hObject,'redraw');
    return
  end
  MASKMAP = load(mapfile);
  fname = fieldnames(MASKMAP);
  MASKMAP = MASKMAP.(fname{1});
  if iscell(MASKMAP),  MASKMAP = MASKMAP{1};  end

  if isfield(MASKMAP,'p'),  MASKMAP = rmfield(MASKMAP,'p');  end
  
  
  ANAP = getappdata(wgts.main,'ANAP');
  permute_vec = ANAP.mnview.permute;
  if ~isempty(permute_vec),
    MASKMAP.dat = permute(MASKMAP.dat, permute_vec);
  end
  
  if strcmpi(eventdata,'edit-maskmap'),
    if isfield(MASKMAP,'datname'),
      switch lower(MASKMAP.datname),
       case {'cv'}
        MINV = 0;   MAXV = 0.05;
       case {'sn','snr'}
        MINV = 20;  MAXV = Inf;
       otherwise
        MINV = round(min(MASKMAP.dat(:)));  MAXV = round(max(MASKMAP.dat(:))*0.7);
      end
    else
      MINV = round(min(MASKMAP.dat(:)));  MAXV = round(max(MASKMAP.dat(:))*0.7);
    end
    % set min/max value for the mask
    set(wgts.MaskmapMinMaxEdt,'string',sprintf('%g  %g',MINV,MAXV));
  else
    tmpv = str2num(get(wgts.MaskmapMinMaxEdt,'string'));
    MINV = tmpv(1);  MAXV = tmpv(2);
  end

  tmpdat = ones(size(MASKMAP.dat),'int16');
  tmpdat(MASKMAP.dat(:) < MINV) = 0;
  tmpdat(MASKMAP.dat(:) > MAXV) = 0;
  
  MASKMAP.dat = tmpdat;
  
  setappdata(wgts.main,'MASKMAP',MASKMAP);
 ...
 case {'slider-sagital'}
  ...
  if ~isempty(GRAHANDLE),
    ...
    if ~isempty(STATMAP) && get(wgts.StatmapCheck,'Value'),
      ...
      if get(wgts.MaskmapCheck,'Value') > 0 && ~isempty(MASKMAP)
        tmpana = MASKMAP.dat(iX,:,:);
        idx = find(tmpana == 0);
        tmps(idx) = 0;
        tmpp(idx) = 1;
      end
 ...
 case {'redraw'}
  ...
  for N = 1:length(SLICES),
    ...
    if ~isempty(STATMAP) && get(wgts.StatmapCheck,'Value'),
      ...
      if get(wgts.MaskmapCheck,'Value') > 0 && ~isempty(MASKMAP)
        if iDimension == 1,
          tmpmsk = squeeze(MASKMAP.dat(iSlice,:,:));
        elseif iDimension == 2,
          tmpmsk = squeeze(MASKMAP.dat(:,iSlice,:));
        else
          tmpmsk = squeeze(MASKMAP.dat(:,:,iSlice));
