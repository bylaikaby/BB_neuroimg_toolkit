...
if isfield(Ses.anap,'mnrealign'),
  if isfield(Ses.anap.mnrealign,'export') & ~isempty(Ses.anap.mnrealign.export),
    DO_EXPORT = Ses.anap.mnrealign.export;
  end
  if isfield(Ses.anap.mnrealign,'use_edges') & ~isempty(Ses.anap.mnrealign.use_edges),
    USE_EDGES = Ses.anap.mnrealign.use_edges;
  end
  if isfield(Ses.anap.mnrealign,'realign') & ~isempty(Ses.anap.mnrealign.realign),
    DO_REALIGN = Ses.anap.mnrealign.realign;
  end
  if isfield(Ses.anap.mnrealign,'reslice') & ~isempty(Ses.anap.mnrealign.reslice),
    DO_RESLICE = Ses.anap.mnrealign.reslice;
  end
  if isfield(Ses.anap.mnrealign,'confirm') & ~isempty(Ses.anap.mnrealign.confirm),
    DO_CONFIRM = Ses.anap.mnrealign.confirm;
  end
end
...
if DO_EXPORT,
  if isfield(Ses.anap,'mnrealign') & isfield(Ses.anap.mnrealign,'datname') & ...
        ~isempty(Ses.anap.mnrealign.datname),
    datname = Ses.anap.mnrealign.datname;
  else
    datname = 'tcImg';
    %datname = '2dseq';
  end
  fprintf(' %s: mn_dat2spm() exporting %s for SPM...\n',gettimestring,datname);
  IMGFILES = mn_dat2spm(Ses,grp.name,datname,0);
else
...
FLAGS = subUpdateFlags(Ses,FLAGS,SPMFLAGS);
...
function flags = subUpdateFlags(Ses,flags,SPMFLAGS),
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(Ses.anap,'mnrealign'),
  if isfield(Ses.anap.mnrealign,'spm_realign'),
    fnames = fieldnames(Ses.anap.mnrealign.spm_realign);
    for N = 1:length(fnames),
      flags.spm_realign.(fnames{N}) = Ses.anap.mnrealign.spm_realign.(fnames{N});
    end
  end
  if isfield(Ses.anap.mnrealign,'spm_reslice'),
    fnames = fieldnames(Ses.anap.mnrealign.spm_reslice);
    for N = 1:length(fnames),
      flags.spm_reslice.(fnames{N}) = Ses.anap.mnrealign.spm_reslice.(fnames{N});
    end
  end
end
...
for N = 1:length(IMGFILES),
  ...
  ExpNo = str2num(fr(strfind(fr,'_')+1:end));
