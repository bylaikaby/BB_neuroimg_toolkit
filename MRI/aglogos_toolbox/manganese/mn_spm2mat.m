...
%    0.97 18.11.2010 YM  supports SPM2/SPM5/SPM8.
%    0.98 10.01.2012 YM  supports sesversion()>=2.
%
...
if DO_EXPORT_GRPTCIMG && ~strcmpi(grp.name,'mdeftinjir'),
  ...
  if exist('tcimg.mat','file') ~= 0 && any(strcmpi(who('-file','tcimg.mat'),SigName)),
...
if DO_EXPORT_ANATOMY,
  ...
  if sesversion(Ses) >= 2,
    anafile = sigfilename(Ses,anascan,ananame);
    ANA = load(anafile,ananame);
    ANA = ANA.(ananame);
    ANA.dat = ADAT;
    ANA.ds  = [xres yres zres];
