...
%    0.97 17.01.12 YM  return empty data when pca-denoised data were not available.
%    0.98 06.02.12 YM  use mroi_file().
%
...
for iRoi = 1:length(ROI.roi),
  ...
  if USE_PCA == 0,
    ...
    if ~isfield(tcImg,'pca_denoised') || isempty(tcImg.pca_denoised),
      fprintf('\n WARNING %s: tcImg.pca_denoised not found.',mfilename);
      fprintf(' Run mndenoise_pca() first.\n');
      if nargout,  varargout{1} = roiTs;  end
      return;
      %tmpdat = nan(size(tcImg.dat));
    else
