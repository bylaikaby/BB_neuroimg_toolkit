function varargout = mncvmap(SESSION,GRPNAME,varargin)
%MNCVMAP - Creates a statistical map of CV.
%  MNCVMAP(SES,GRPNAME,...) creates a statistical map of CV.
%  Coefficient of Variation (CV) = std/mean
%
%  Supported options are :
%    'twin'   : time window (indices) to compute the CV map
...
%    ANAP.mncvmap.use_realigned = 1;
%    ANAP.mncvmap.smooth        = 0;
%    ANAP.mncvmap.twin          = [1:6];
%
%  EXAMPLE :
%    >> mncvmap('rat361','mdeftinj','twin',[1:6])
%    >> mnview('rat361','mdeftinj')
%
%  VERSION :
%    0.90 16.09.10 YM  pre-release
%
%  See also mnview mnsnmap
...
% check ANAP
if isfield(anap,'mncvmap'),
  tmpanap = anap.mncvmap;
...
if DO_SMOOTH > 0,
  ...
  for iSlice = 1:nS,
    ...
    % compute CV
    if SAVEAS_MATFILE > 0,
      tmpcv = subComputeCV(tcProc,USE_PCA,MAP_TWIN);
      CSTAT(:,:,iSlice) = tmpcv.cv;
      PVAL(:,:,iSlice)  = tmpcv.p;
    end
  end
  ...
  for iSlice = 1:nS,
    ...
    if SAVEAS_MATFILE > 0,
      ...
      tmpcv = subComputeCV(tcProc,0,MAP_TWIN);
      tmpcv.smooth  = DO_SMOOTH;
      tcImg.cvmap = tmpcv;
      if isfield(tcImg,'pca_denoised') & ~isempty(tcImg.pca_denoised),
        tmpttest = subComputeCV(tcProc,1,MAP_TWIN);
        tcImg.cvmap.pca_p  = tmpcv.p;
        tcImg.cvmap.pca_cv = tmpcv.cv;
      end
      if USE_PCA > 0,
        CSTAT(:,:,iSlice) = tcImg.cvmap.pca_cv;
        PVAL(:,:,iSlice)  = tcImg.cvmap.pca_p;
      else
        CSTAT(:,:,iSlice) = tcImg.cvmap.cv;
        PVAL(:,:,iSlice)  = tcImg.cvmap.p;
      end
      CSTAT(:,:,iSlice) = tmpcv.cv;
      PVAL(:,:,iSlice)  = tmpcv.p;
      save(matfile,'tcImg');
...
STATS.mapname = 'cvmap';
STATS.datname = 'cv';
STATS.tsel    = tmpcv.tsel;
STATS.dat     = CSTAT;			% t statistics
...
if nargout,
  ...
  if SAVEAS_MATFILE > 0,
    matfile = sprintf('cvmap_realign(%d)_smooth(%d).mat',...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBFUNCTION to compute CV
function CVMAP = subComputeCV(tcImg,USE_PCA,TWIN)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if USE_PCA,
  datname = 'pca_denoised';
else
...
CVMAP.tsel  = TWIN;
CVMAP.p     = reshape(P,szdat(1:end-1));
CVMAP.cv    = reshape(C,szdat(1:end-1));
