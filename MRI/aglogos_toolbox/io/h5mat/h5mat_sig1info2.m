function [sinfo vinfo] = h5mat_sig1info2(Filename,SigName,varargin)
%
%
%  NOTE :
%    Trying to improve reading speed, although it looks similar as siginfo()...
%
%  VERSION :
%    0.90 11.06.13 YM  pre-release
%
%  See also siginfo
...
try  
  [sinfo vinfo] = get_siginfo(Filename, location, fields);
  sinfo.signame = SigName;
  n = numel(fieldnames(sinfo));
  sinfo = orderfields(sinfo, [n 1:n-1]);
...
% ----------------------------------------------------------------------
function [sinfo vinfo] = get_siginfo(filename, location, FIELDS)
...
switch(link_info.type)
 ...
 case H5ML.get_constant_value('H5L_TYPE_HARD')
  ...
  vinfo = get_toplevel_hardlink_info(fid,location);
                   
 case H5ML.get_constant_value('H5L_TYPE_SOFT')
  vinfo = get_link_info(fid,location,link_info.type);
...
vinfo = sub_matclass_size(vinfo);

vinfo.Filename = fullhfile;
% Reorder the fields such that Filename is on top.
n = numel(fieldnames(vinfo));
vinfo = orderfields(vinfo, [n 1:n-1]);

switch lower(vinfo.class)
 case {'struct'}
  sinfo = sub_sig1load(fid,vinfo,FIELDS);
 case {'cell'}
  tmpinfo = vinfo;
  while strcmpi(tmpinfo.class,'cell')
    if prod(tmpinfo.size) > 1
      tmpinfo = sub_deref_info(fid,tmpinfo.Name,[1 1],[1 1]);
    else
      tmpinfo = sub_deref_info(fid,tmpinfo.Name);
    end
    tmpinfo = sub_matclass_size(tmpinfo);
  end
  sinfo = sub_sig1load(fid,tmpinfo,FIELDS);
...
% ----------------------------------------------------------------------
function hinfo = sub_matclass_size(hinfo)

hinfo.class = 'unknown';
hinfo.size  = [];
n = numel(fieldnames(hinfo));
hinfo = orderfields(hinfo, [n-1 n 1:n-2]);
...
%--------------------------------------------------------------------------
function hinfo = sub_deref_info(fid,Location,varargin)
hinfo = [];
...
switch (obj_info.type)
 ...
 case H5ML.get_constant_value('H5O_TYPE_DATASET')
  ...
  try
    dataset_id    = H5D.open(fid,Location,dapl);
  catch me
    if strcmp(me.identifier,'MATLAB:imagesci:hdf5lib:libraryError')
      error(message('MATLAB:imagesci:h5read:datasetDoesNotExist', Location, Filename));
    else
      rethrow(me);
    end
  end
...
%--------------------------------------------------------------------------
function data = sub_h5read(fid,Dataset,varargin)
p = inputParser;
...
switch var_class
    ...
    case H5ML.get_constant_value('H5T_REFERENCE')
        data = h5postprocessreferences(dataset_id,dataspace,rawData);
...
%--------------------------------------------------------------------------
function sinfo = sub_sig1load(fid,vinfo,FIELDS)
sinfo = [];
...
for N = 1:length(fnames)
  ...
  if any(start),
    tmpinfo = sub_deref_info(fid,tmppath,start,count);
  else
    tmpinfo = sub_deref_info(fid,tmppath);
  end
  ...
  if strcmpi(tmpinfo.class,'cell'),  continue;  end
  
  if strcmpi(tmpf,'dat'),
    sinfo.datsize = tmpinfo.Dataspace.Size;
  else
    if any(start)
      tmpv = sub_h5read(fid,tmppath,start,count);
    else
      tmpv = sub_h5read(fid,tmppath);
    end
    if any(count)
      if iscell(tmpv) && numel(tmpv) == 1,  tmpv = tmpv{1};  end
    end
    switch lower(tmpinfo.class),
     case {'char'}
      tmpv = char(tmpv);
     otherwise
    end
