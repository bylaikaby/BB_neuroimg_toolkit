function im = read2dseq(fname,nx,nx1,nx2,ny,ny1,ny2,ns,ns1,ns2,nt1,nt2,byteorder,wordtype,datclass,SHOW_WAITBAR)
...
%   YM,  05.11.08 fixed 'Warning: Unable to interpret TeX string' for waitbar.
%
...
if ~exist('SHOW_WAITBAR','var'), SHOW_WAITBAR = 1;  end
...
if nargin == 1, 
  ...
  im = subRead2dseq(fname,nx,nx1,nx2,ny,ny1,ny2,ns,ns1,ns2,nt1,nt2,...
                    byteorder,wordtype,datclass,SHOW_WAITBAR);
end
...
function im = subRead2dseq(fname,nx,nx1,nx2,ny,ny1,ny2,ns,ns1,ns2,nt1,nt2,byteorder,wordtype,dattyp,SHOW_WAITBAR)
...
if any(SHOW_WAITBAR),
  h = waitbar(0,strrep(sprintf('read2dseq: Loading images...\n %s',strrep(fname,'\','/')),'_','\_'));
else
  h = [];
end
...
try
  ...
  for TPNT = 1:nt,
    if ishandle(h),  waitbar(TPNT/nt, h);  end
