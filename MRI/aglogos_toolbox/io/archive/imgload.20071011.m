...
%
% NOTE :
%  Setting parameters can be controlled by ANAP.imgload.xxx.
%
...
%           1.07 31.08.07 YM   support ISUBSTITUDE_RAND.
%
...
if strcmpi(pvpar.reco.RECO_byte_order,'bigEndian'),
  ...
  img=read2dseq(tcImg.dir.imgfile,nx,x1,x2,ny,y1,y2,ns,ns1,ns2,t1,t2,...
                's',pvpar.reco.RECO_wordtype,IDATCLASS);
else
  % no swap for INTEL
  img=read2dseq(tcImg.dir.imgfile,nx,x1,x2,ny,y1,y2,ns,ns1,ns2,t1,t2,...
                'n',pvpar.reco.RECO_wordtype,IDATCLASS);
end
...
if ISUBSTITUDE,
  fprintf(' substituting(%d,rand=%d).',ISUBSTITUDE,ISUBSTITUDE_RAND);
  for NS=NS1:NS2,
    ...
    if ISUBSTITUDE_RAND, 
      ...
      for x = 1:size(img,1),
        for y = 1:size(img,2),
          img(x,y,NS,idx) = img(x,y,NS,idx(randperm(ISUBSTITUDE)));
        end
      end
    end
  end;
