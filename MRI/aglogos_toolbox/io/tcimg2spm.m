...
%    0.96 09.04.20 YM  support 'UndoCrop' and 'FlipDim'.
%
...
if any(UNDO_CROP)
  p = expgetpar(Ses,tcImg.ExpNo(1));
  grp = getgrp(Ses,tcImg.ExpNo(1));
  newdat = zeros([p.pvpar.nx, p.pvpar.ny, p.pvpar.nsli, p.pvpar.nt]);
  if ~isequal(size(newdat),size(tcImg.dat)) && ~isempty(grp.imgcrop)
    IMGCROP = grp.imgcrop;
    ix = (0:IMGCROP(3)-1) + IMGCROP(1);
    iy = (0:IMGCROP(4)-1) + IMGCROP(2);
    newdat(ix,iy,:,:) = tcImg.dat;
    tcImg.dat = newdat;
  end
  clear newdat;
end
