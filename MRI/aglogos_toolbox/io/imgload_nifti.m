function OtcImg = imgload_nifti(SESSION,ExpNo,ARGS)
%IMGLOAD_NIFTI - Load NIFTI file as tcImg
%
% VERSION :
%   0.90 23.07.10 YM   pre-release
%   0.91 14.03.11 YM   supports "IDATCLASS".
%   0.92 05.10.13 YM   modified to support nifti from phased-array exps.
%
% See also SESIMGLOAD IMGLOAD SPM_VOL SPM_VOL_READ
...
tcImg = sub_mgettcimg(Ses,ExpNo,imgpar);
...
fprintf(' imgload_nifti: ');

% IF CROPPING REQUIRED GET DIMENSIONS FROM PAR-FILE
% THEY ORIGINALLY DEFINED IN Ses.grp.name.crop
if ICROP && isfield(grp,'imgcrop') && ~isempty(grp.imgcrop),
  x1 = grp.imgcrop(1);
  y1 = grp.imgcrop(2);
  x2 = x1 + grp.imgcrop(3) - 1;
  y2 = y1 + grp.imgcrop(4) - 1;
  fprintf('imgcrop[%dx%d->%dx%d].',nx,ny,grp.imgcrop(3),grp.imgcrop(4));
else
  x1 = 1;	y1 = 1;
  x2 = nx;	y2 = ny;
  fprintf('imgsize[%dx%dx%d].',nx,ny,ns);
end;
%t1	= tcImg.usr.imgofs;
...
if ISLICROP > 0,
  slicrop = [];
  if isfield(Ses.expp(ExpNo),'slice') && ~isempty(Ses.expp(ExpNo).slice),
    % given as [start end] like [16 30], 15 slices of 16:30
    slicrop = [Ses.expp(ExpNo).slice(1) Ses.expp(ExpNo).slice(2)-Ses.expp(ExpNo).slice(1)+1];
  elseif isfield(Ses.expp(ExpNo),'slicrop') && ~isempty(Ses.expp(ExpNo).slicrop),
...
fprintf(' NIFTI(nt=%d).',imgpar.nt)

img = sub_read_nifti(Ses,ExpNo,imgpar,x1,x2,y1,y2,ns1,ns2);

if ~isa(img,IDATCLASS),
  fprintf(' %s->%s.',class(img),IDATCLASS);
  eval(sprintf('img = %s(img);',IDATCLASS));
end
...
if ISUBSTITUTE,
  fprintf(' substitute(%d,rand=%d).',ISUBSTITUTE,ISUBSTITUTE_RAND);
  for NS=1:size(img,3),
  ...
  if tmpdat(1)-tmpm > tmps*10,
    ...
    line(get(gca,'xlim'),[tmpm tmpm]-1*tmps,'color','r');
    title(sprintf('%s: ExpNo=%d(%s)',Ses.name,ExpNo,grp.name));
    fprintf('\n WARNING %s: %s Exp=%d(%s) initial transients detected, not enough dummies...',mfilename,Ses.name,ExpNo,grp.name);
    fprintf('\n Please set ANAP.imgload.ISUBSTITUTE (for all) or GRP.%s.anap.imgload.ISUBSTITUTE (for the group) in volumes.\n',grp.name);
    %keyboard
  end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function tcImg = sub_mgettcimg(Ses,ExpNo,imgpar)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tcImg.dir.scanreco	= '';
tcImg.dir.imgfile	= expfilename(Ses,ExpNo,'nifti');
...
tcImg.ds	= [imgpar.res(1) imgpar.res(2) imgpar.slithk];
tcImg.dx	= imgpar.imgtr;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function IMG = sub_read_nifti(Ses,ExpNo,imgpar,x1,x2,y1,y2,ns1,ns2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for K = 1:length(tmpdirs)
  if tmpdirs(K).isdir ~= 0,  continue;  end
  NIIFILE{end+1} = fullfile(fp,tmpdirs(K).name);
end

NIIFILE = sort(NIIFILE);
...
IMG = [];
if length(NIIFILE) > 1,
  % 1 volume for each file
  for K = length(NIIFILE):-1:1,
    tmpvol = spm_vol(NIIFILE{K});
    tmpimg = spm_read_vols(tmpvol);
    % flip the direction of X/Y to match with SPM display or paravision
    tmpimg = flipdim(tmpimg,1);
  ...
  % multi volumes in a single file
  [fp fr fe] = fileparts(NIIFILE{1});
  if strcmpi(fe,'.gz'),
    niifile = gunzip(NIIFILE{1},tempdir());
    tmpvol = spm_vol(niifile{1});
    tmpimg = spm_read_vols(tmpvol);
    for F = 1:length(niifile),  delete(niifile{F});  end
  else
    tmpvol = spm_vol(NIIFILE{1});
    tmpimg = spm_read_vols(tmpvol);
  end
