function MDATA = smr_readevt(SMRFILE,CHAN,varargin)
%SMR_READWAV - Read Event data from a SPIKE2 SMR file.
%  MDATA = SMR_READWAV(SMRFILE,CHAN,...) reads Event data form a SPIKE2 SMR file.
...
if isempty(chheader)
    ...
    if isempty(WINDOW_SEC) || ~any(WINDOW_SEC),
        ...
        MDATA.times  = data(data > WINDOW_SEC(1) & data < sum(WINDOW_SEC));
