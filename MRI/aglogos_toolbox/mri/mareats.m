...
%    1.16 04.02.21 YM  supports IDETREND_METHOD (linear|spline)
%
...
DEF.ISUBSTITUDE         = 0;
DEF.IRESAMPLE           = 0;
DEF.IDETREND            = 1;        % detrending
DEF.IDETREND_METHOD     = 'linear'; % detrending method: linear or spline
DEF.IDETREND_SPLINE_SEC = 60;       % spline segment in seconds
...
if IREMBRAINMEAN && MB_METHOD
  % force to detrend
  IDETREND = 1;  ARGS.IDETREND = 1;
end


% DETREND DATA:  HERE to work with/without IREMBRAINMEAN-MB_METHOD
if any(IDETREND)
  switch lower(IDETREND_METHOD)
   case {'spline'}
    fprintf(' detrend(%s,%gs).',IDETREND_METHOD,IDETREND_SPLINE_SEC);
    roiTs = sigdetrend_spline(roiTs,'keepDC',0,'seg_sec',IDETREND_SPLINE_SEC);
   otherwise
    fprintf(' detrend(linear).');
...
if IRESAMPLE > 0 && (IRESAMPLE ~= roiTs{1}.dx)
  RESAMPLE_ONLY = 0;
  if RESAMPLE_ONLY
