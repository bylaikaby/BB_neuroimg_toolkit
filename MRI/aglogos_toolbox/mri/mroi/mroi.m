...
%    1.55 24.01.20 YM  new: more precise coordinates for new sessions.
%
...
if strcmpi(get(wgts.AtlasSliceBarSldr,'Enable'),'on')
 ...
 case {'load'}
  ...
  if exist(RoiFile,'file') && ~isempty(who('-file',RoiFile,RoiVar))
    ...
    if isfield(Roi,'CONV_PRECISE_COORDS')
      CONV_PRECISE_COORDS = Roi.CONV_PRECISE_COORDS;
    else
      CONV_PRECISE_COORDS = 0;  % to keep the compatibility to old sessions/ROIs.
    end
  ...
  setappdata(wgts.main,'CONV_PRECISE_COORDS',CONV_PRECISE_COORDS);
 ...
 case {'save'}
  ...
  Roi.CONV_PRECISE_COORDS = getappdata(wgts.main,'CONV_PRECISE_COORDS');
  % if isempty(Roi.roi),
 ...
 case {'coordinate'}
  ...
  while 1
    ...
    if wgts.ImageAxs == gca
      ...
      IMGDIMS = getappdata(wgts.main,'IMGDIMS');
      tmpxy(1) = (tmpxy(1)-0.5) * IMGDIMS.pxscale + 0.5;
      tmpxy(2) = (tmpxy(2)-0.5) * IMGDIMS.pyscale + 0.5;
    end
...
CONV_PRECISE_COORDS = getappdata(wgts.main,'CONV_PRECISE_COORDS');
...
switch lower(COMMANDSTR)
 ...
 case {'erase bitmap'}
  ...
  while 1
    ...
    if any(CONV_PRECISE_COORDS)
      % new: more preceise but may lose the compatibility
      RoiErase{N}.px    = (anapx-0.5) * IMGDIMS.pxscale + 0.5;
      RoiErase{N}.py    = (anapy-0.5) * IMGDIMS.pyscale + 0.5;
    else
 ...
 case {'midline'}
  ...
  for N = 1:1
    ...
    RoiML{K}.x  = round((RoiML{K}.anax-0.5) * IMGDIMS.pxscale + 0.5);
    RoiML{K}.y  = round((RoiML{K}.anay-0.5) * IMGDIMS.pyscale + 0.5);
    % plot the position
    set(wgts.main,'CurrentAxes',wgts.ImageAxs);  hold on;
 ...
 case {'ant.commisure'}
  ...
  for N = 1:1
    ...
    RoiAC{K}.x  = round((RoiAC{K}.anax-0.5) * IMGDIMS.pxscale + 0.5);
    RoiAC{K}.y  = round((RoiAC{K}.anay-0.5) * IMGDIMS.pyscale + 0.5);
    % plot the position
    set(wgts.main,'CurrentAxes',wgts.ImageAxs);  hold on;
    plot(x,y,'yx','markersize',12);
 ...
 case {'lr-separate'}
  ...
  for N = 1:1
    ...
    RoiLR{K}.x  = round((anapx-0.5) * IMGDIMS.pxscale + 0.5);
    RoiLR{K}.y  = round((anapy-0.5) * IMGDIMS.pyscale + 0.5);
    % plot the position
    set(wgts.main,'CurrentAxes',wgts.ImageAxs);  hold on;
 ...
 case {'electrodes'}
  ...
  for N = 1:length(grp.hardch)
    ...
    RoiEle{K}.x  = round((RoiEle{K}.anax-0.5) * IMGDIMS.pxscale*0.5);
    RoiEle{K}.y  = round((RoiEle{K}.anay-0.5) * IMGDIMS.pyscale*0.5);
    % plot the position
    set(wgts.main,'CurrentAxes',wgts.ImageAxs);  hold on;
...
switch lower(COMMANDSTR)
 ...
 case {'lus','ios','sts','ls','ips','cs','as','ps',...
  ...
  for N = 1:1
    ...
    RoiMarker{K}.x  = round((RoiMarker{K}.anax-0.5) * IMGDIMS.pxscale + 0.5);
    RoiMarker{K}.y  = round((RoiMarker{K}.anay-0.5) * IMGDIMS.pyscale + 0.5);
    % plot the position
    set(wgts.main,'CurrentAxes',wgts.ImageAxs);  hold on;
...
for N = 1:length(Roi.roi)
  ...
  if floor(max(Roi.roi{N}.px)) > max(px)+1 && floor(max(Roi.roi{N}.py)) > max(py)+1
    ...
    % % now convert anatomy to functional
    % Roi.roi{N}.px      = Roi.roi{N}.px * IMGDIMS.pxscale;
    % Roi.roi{N}.py      = Roi.roi{N}.py * IMGDIMS.pyscale;

    % % first convert for the current anatomy
    Roi.roi{N}.px      = (Roi.roi{N}.px-0.5) * IMGDIMS.ana(1) / IMGDIMS.anaorig(1) + 0.5;
    Roi.roi{N}.py      = (Roi.roi{N}.py-0.5) * IMGDIMS.ana(2) / IMGDIMS.anaorig(2) + 0.5;
    % % now convert anatomy to functional
    Roi.roi{N}.px      = (Roi.roi{N}.px-0.5) * IMGDIMS.pxscale + 0.5;
    Roi.roi{N}.py      = (Roi.roi{N}.py-0.5) * IMGDIMS.pyscale + 0.5;
...
if isfield(Roi,'ele')
  for N = 1:length(Roi.ele)
    ...
    tmpx = (Roi.ele{N}.x-0.5) * IMGDIMS.pxscale + 0.5;
    if tmpx ~= Roi.ele{N}.anax
      % Roi.ele{N}.anax = Roi.ele{N}.x / IMGDIMS.pxscale;
      % Roi.ele{N}.anay = Roi.ele{N}.y / IMGDIMS.pyscale;
      Roi.ele{N}.anax = (Roi.ele{N}.x-0.5) / IMGDIMS.pxscale + 0.5;
      Roi.ele{N}.anay = (Roi.ele{N}.y-0.5) / IMGDIMS.pyscale + 0.5;
    end
  end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBFUNCTION to convert coordinates
function XY2 = subConvertCoords(XYSIZE1,XYSIZE2,XY1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
XY2(1) = interp1([0.5 XYSIZE1(1)+0.5],[0.5 XYSIZE2(1)+0.5],XY1(1),'linear','extrap');
XY2(2) = interp1([0.5 XYSIZE1(2)+0.5],[0.5 XYSIZE2(2)+0.5],XY1(2),'linear','extrap');
...
for N = 1:length(grpnames)
  ...
  if isfield(tmpgrp,'ele') && isfield(tmpgrp.ele,'mcoords') && ~isempty(tmpgrp.ele.mcoords)
    ...
    for N = 1:size(tmpgrp.ele.mcoords,1)
      ...
      % new: more preceise but may lose the compatibility
      RoiEle{N}.anax  = (tmpgrp.ele.mcoords(N,1)-0.5) / IMGDIMS.pxscale + 0.5;
      RoiEle{N}.anay  = (tmpgrp.ele.mcoords(N,2)-0.5) / IMGDIMS.pyscale + 0.5;;
...
for N = 1:length(RoiRoi)
  ...
  if any(CONV_PRECISE_COORDS)
    px      = (RoiRoi{N}.px-0.5) / IMGDIMS.pxscale + 0.5;
    py      = (RoiRoi{N}.py-0.5) / IMGDIMS.pyscale + 0.5;
  else
