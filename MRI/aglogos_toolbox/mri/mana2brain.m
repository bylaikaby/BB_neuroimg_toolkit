...
%    'brain_mask' :  0|1, use "Brain" mask in ROI.
...
%    GRP.xxx.anap.mana2brain.brain_mask = 0;
...
%    0.97 24.02.20 YM  supports 'brain_mask'.
%    0.98 17.09.20 YM  'brain_mask' works only when 'epi'.
%
...
if exist('Ses','var') && exist('GrpName','var')
  ...
  if isfield(anap,'mana2brain')
    ...
    if isfield(x,'brain_mask')
      BRAIN_MASK = x.brain_mask;
    end
...
for N = iOPT:2:length(varargin)
  switch lower(varargin{N})
   ...
   case {'mask' 'brain_mask' 'brainmask'}
    BRAIN_MASK = varargin{N+1};
   case {'export'}
...
if exist('Ses','var') && exist('GrpName','var')
  IMGFILES = subAnaExport(Ses,GrpName,USE_EPI,USE_RAW,BRAIN_MASK,DO_EXPORT,'brain',IS_RAT);
end
...
if any(USE_EPI) && any(BRAIN_MASK)
  if any(strfind(lower(BRAIN_TYPE),'(brain)')) || any(strfind(lower(BRAIN_TYPE),'[brain]'))
  else
    BRAIN_TYPE = [BRAIN_TYPE '(brain)'];
  end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [IMGFILES, Ses, GrpName] = subAnaExport(Ses,Grp,USE_EPI,USE_RAW,BRAIN_MASK,DO_EXPORT,DIR_NAME,IS_RAT)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ses = goto(Ses);
if ~exist('Grp','var')
  Grp = getgrp(Ses);
...
for N = 1:length(ananame)
  ...
  if any(BRAIN_MASK) && any(strcmpi(anafile{N},'epi'))
    fprintf(' brain_mask.');
    [roifile,roivar] = mroi_file(Ses,anagrpn{N});
    ROI = load(roifile,roivar);
    ROI = ROI.(roivar);
    BMASK = zeros(size(ANA.dat));
    BMASK = reshape(BMASK,[size(BMASK,1)*size(BMASK,2) size(BMASK,3)]);
    for R = 1:length(ROI.roi)
      if strcmpi(ROI.roi{R}.name,'Brain')
        tmpmask = ROI.roi{R}.mask;
        if size(tmpmask,1) ~= size(ANA.dat,1) || size(tmpmask,2) ~= size(ANA.dat,2)
          tmpmask = imresize(tmpmask,[size(ANA.dat,1) size(ANA.dat,2)],'nearest');
        end
        tmpi = (tmpmask(:) > 0);
        BMASK(tmpi,ROI.roi{R}.slice) = 1;
      end
    end
    BMASK = reshape(BMASK,size(ANA.dat));
    % for n = 1:size(BMASK,3), subplot(5,5,n); imagesc(squeeze(BMASK(:,:,n))'); end
    ANA.dat = ANA.dat .* BMASK;
  end
