...
%
%01/04/04 Uses the noise variance drawing procedure of code from 1st year report.
...
DEF.SIGMASQ_A       = 1;       
DEF.SIGMASQ_B       = 1000;    
DEF.SIG_TAUSQPVAR   = 0.005; 
DEF.SIG_SIGMASQPVAR = 0.05;
...
%Starting parameters
ARamp=ARcoeffGen(0.9*ones(1,NP/2),[3*pi/4 pi/2 pi/4]);
%  ARamp = ARamp0;
tauSq=2;   %DEBUG: fixed at true value
sigmaSq=sd^2;
...
for j=1:NITERATIONS
  if j>50   %Don't draw new AR parameters until noise terms stabilise
