...
for VoxNo = 1:size(Sig.dat,2),
  ...
  %Look at signal spectrum, normalised sinusoid magnitude profile vs phase, to see which
  %gives an indicator of which component can be removed. Use 1st harmonic as test since it should have
  %biggest component.
  % For component 1, seems 7 is a good threshold
  freqaxis = [-pi:2*pi/N:pi-pi/N];  
  breathe_w = w_array*Sig.dx;   %breathing freqs in radians
  amp_w = (( ones(4,1)*freqaxis ) < ( breathe_w*ones(1,length(freqaxis)) ))   .* (ones(4,1)*freqaxis);
  ...
  if max(a(a_ind(1:2))) > 2*mean(a(a_ind(2)+1:a_ind(2)+10))
    fprintf('removing... %6.3f %6.3f \n ',max(a(a_ind(1:2))),2*mean(a(a_ind(2)+1:a_ind(2)+10)))
...
%  if 0
  ...
  subplot(1,2,1);
  plot(freqaxis(plotIndex),a(plotIndex),freqaxis(a_ind),a(a_ind),'*r');
  subplot(1,2,2);
  a = abs(dArray(:,2));
