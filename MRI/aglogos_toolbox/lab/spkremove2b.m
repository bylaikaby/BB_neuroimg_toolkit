...
%This version works on AVERAGE OVER ALL VOXELS (compared with spkremove2.m)
...
%Sig.dat =Sig.dat(:,323);  %320 is an example with well defined spikes
...
%DEBUG
%keyboard
Sig.dat = breathe_remove_MUSIC(Sig.dat,[0.619 1.2365 2.473],length(Sig.dat));
...
DEF.NOUTLIERS       = 10;
DEF.OTLSCALE        = 20000000;  %debug: outlier scaling factor
...
DEF.SIGMASQ_A       = 1e-3;       
DEF.SIGMASQ_B       = 1000;    
DEF.SIG_TAUSQPVAR   = 0.0000005; 
DEF.SIG_SIGMASQPVAR = 0.0000005;
...
%ARamp = [0.7151    0.0837    0.1123   -0.0654    0.7483   -0.6632    0.1034   -0.0890]';
ARamp = [0.6731    0.0063    0.3207   -0.2045    0.6637   -0.5257    0.2957   -0.2756];
%  ARamp = ARamp0;
tauSq=3.56e-4;   %
sigmaSq=0.0013;         %sd^2;
...
for j=1:NITERATIONS
  ...
  if j>0   %Don't draw new AR parameters until noise terms stabilise
   ...
   %DEBUG: use *true* outliers
%   outlIndArray = zeros(NPTS,1);
%   outlIndArray([27 1186])= [1 1];
   
   %update standard devs of y
   yStds = sqrt(sigmaSq) * ( (sqrt(spikeVarScale)-1)*outlIndArray + 1);
   fprintf('Num outliers: %i\n',sum(outlIndArray));
