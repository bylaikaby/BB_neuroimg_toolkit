...
GROUPS = {};
GROUPS{end+1} = 'd04lg1_14';
GROUPS{end+1} = 'd04fu1_11';
GROUPS{end+1} = 'd04g11_07';
GROUPS{end+1} = 'd04j51_11';
GROUPS{end+1} = 'd04jq1_11';
GROUPS{end+1} = 'd04k41_04';
GROUPS{end+1} = 'd04mf1_04';
...
xticklabel = {};
for N = 1:length(GROUPS),
  if any(strfind(GROUPS{N},'d04lg1')),
    % 7T test scan
    idx_7T(end+1) = N;
...
if length(idx_7T) > 1,
  ...
  [h,signifP,ci] = ztest(tmpm(1,1),tmpm(2,1),tmps(2,1));
  [h,signifN,ci] = ztest(tmpm(1,2),tmpm(2,2),tmps(2,2));
end
...
if length(idx_7T) > 1,
  ...
  legend(sprintf('CorrR>0 P=%.3f(z-test)',signifP),...
         sprintf('CorrR<0 P=%.3f(z-test)',signifN),'location','northwest');
end
ylabel('# of Activated Voxels');
...
if length(idx_7T) > 1,
  ...
  legend(sprintf('CorrR>0 P=%.3f(z-test)',signifP),...
         sprintf('CorrR<0 P=%.3f(z-test)',signifN),'location','southeast');
end
ylabel('BOLD Amplitude (%)');
set(gca,'ylim',[0 max(get(gca,'ylim'))]);
set(gca,'xlim',[0.5 2.5],'xtick',[1 2],'xticklabel',[]);
