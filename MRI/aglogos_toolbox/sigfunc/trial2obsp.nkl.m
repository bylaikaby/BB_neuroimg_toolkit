...
if nargout == 0,
  ...
  elseif strcmpi(SigName,'ClnSpc'),
    mfigure([10 100 800 600]);
    dspclnspc(oSig);
  else
    mfigure([10 100 900 800]);
    subplot(2,1,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
CLNSPC=0;
if strcmpi(SigName,'ClnSpc') | strcmpi(SigName,'tClnSpc'),
  CLNSPC=1;
end;
...
if ~any(strcmpi(SigName,{'roiTs','troiTs','blp','model','ClnSpc'})),
  error('ERROR %s: ''%s'' is not supported yet.',mfilename,SigName);
  return;
end

if strcmpi(SigName,'tblp') | strcmpi(SigName,'blp'),
  if DIM==3 | DIM==4 | DIM==5, % Exp/Grp BLP: .dat is Time X Chan X Band X TrialNo X ExpNo for BLP
    for T=1:length(Sig),
      Sig{T}.dat = mean(Sig{T}.dat,DIM);
    end;
    DIM = DIM-1;
  else
    fprintf('trial2obsp: TBLP can have DIM of 3, 4 or 5; Check the signal\n');
    keyboard;
  end;
elseif ~strcmpi(SigName,'blp') & DIM == 4,
  % GROUPED SIGNAL
  ...
  DIM = DIM-1;
elseif strcmp(SigName,'ClnSpc') & DIM == 3,
  % GROUPED SIGNAL
  % .dat is Time X Voxel X TrialNo X ExpNo for (t)roiTs
  for T=1:length(Sig),
    Sig{T}.dat = mean(Sig{T}.dat,DIM);
  ...
  fprintf('trial2obsp: %s, DIM=%d is unexpected\n', SigName, DIM);
  keyboard;
end;
...
switch StatName,
 case {'none'},
  for T=1:length(Sig),
    ...
    for N=1:size(Sig{T}.dat,DIM),
      if strcmpi(SigName,'ClnSpc') & DIM == 3,  % ClnSpc.dat(TimeXFreqXChan)
        oSig.dat = cat(1,oSig.dat,Sig{T}.dat);
      elseif DIM == 3,        % roiTs or other one-band signals
    ...
    % update .stm
    DatLen = size(Sig{T}.dat,1)*Sig{T}.dx;
    Nrep   = size(Sig{T}.dat,DIM);
    oSig.stm = subUpdateSTM(oSig.stm, Sig{T}, Nrep, DatLen, OFFS_STM_TIME);
    OFFS_STM_TIME = size(oSig.dat, 1) * oSig.dx;
  end;
    
 case {'mean'},
  for T=1:length(Sig),
    % update .dat
    if strcmpi(SigName,'blp') | strcmpi(SigName,'blp'),
      oSig.dat = cat(1,oSig.dat,mean(Sig{T}.dat,2)); % Time, Chan, Band
    elseif strcmpi(SigName,'roiTs') | strcmpi(SigName,'troiTs'),
      oSig.dat = cat(1,oSig.dat,Sig{T}.dat); % 
    elseif strcmpi(SigName,'ClnSpc') | strcmpi(SigName,'tClnSpc'),
      oSig.dat = cat(1,oSig.dat,Sig{T}.dat);
    end;
    % update .stm
    if ~CLNSPC, % Never used as regressor
      DatLen = size(Sig{T}.dat,1)*Sig{T}.dx;
 ...
 case {'mean'},
  for T=1:length(Sig),
    % update .dat
    if strcmpi(SigName,'blp') | strcmpi(SigName,'blp'),
      oSig.dat = cat(1,oSig.dat,median(Sig{T}.dat,2)); % Time, Chan, Band
    elseif strcmpi(SigName,'roiTs') | strcmpi(SigName,'troiTs'),
      oSig.dat = cat(1,oSig.dat,Sig{T}.dat); % 
    elseif strcmpi(SigName,'ClnSpc') | strcmpi(SigName,'tClnSpc'),
      oSig.dat = cat(1,oSig.dat,Sig{T}.dat,3);
    end;
    % update .stm
    DatLen = size(Sig{T}.dat,1)*Sig{T}.dx;
    Nrep   = 1;    % should be 1 since it is averaged.
...
if ~CLNSPC,
  ...
  DatLen = size(oSig.dat,1)*oSig.dx;
