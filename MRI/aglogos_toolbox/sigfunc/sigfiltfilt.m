...
%    0.96 27.07.20 YM  added a missing FIR function, clean-up.
%
...
for N = 1:2:length(varargin)
  switch lower(varargin{N})
   ...
   case {'fir_db','firdb','db' 'fir_stopdb' 'stopdb'}
    FIR_dB = varargin{N+1};
...
if ischar(arg2)
  ...
  switch lower(FILTER_NAME)
   ...
   case {'fir'}
    [b, a] = subDesignFIR(nyqf*2, bandHz, ftype,...
...
% ----------------------------------------------------------
function [b,a,pars] = subDesignFIR(Fs,lim,mode,varargin)
% ----------------------------------------------------------

tr         =  1;   % transition bandwidth in Hz
stopdB     = 60;   % stopband attenuation in dB
passripple = 0.01; % ripple in passband in dB

for N = 1:2:length(varargin)
  switch lower(varargin{N})
   case {'fir_passripple','firpassripple','passripple'}
    passripple = varargin{N+1};
   case {'fir_db','firdb','db' 'fir_stopdb' 'stopdb'}
    stopdB = varargin{N+1};
   case {'fir_transband','firtransband','transband'}
    tr = varargin{N+1};
