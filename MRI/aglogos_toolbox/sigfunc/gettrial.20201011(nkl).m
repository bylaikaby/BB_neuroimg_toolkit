...
%    0.95 YM  15.03.19 avoids the error when no anap.gettrial.IBRAINMEAN.
%
...
if ~isfield(ANAP.gettrial,'IBRAINMEAN')
  ANAP.gettrial.IBRAINMEAN = 0;
end
...
for RoiNo=1:length(Sig)                % For all signals (e.g. roiTs with many ROIs)
  ...
  if ~strcmpi(ANAP.gettrial.Xmethod,'none')
    HemoDelay = [];  HemoTail = [];
    if any(strcmpi({'tcImg','roiTs','troiTs','froiTs','tfroiTs'},oSig{RoiNo}{1}.dir.dname))
...
if ~ANAP.gettrial.trial2obsp
  if iscell(oSig), oSig = oSig{1}; end;  return;
end
