...
% of the iINFO.band cell-array below. The LFP (0-90) is provided in a raw-form only for
...
%       6. MUA
%
%      iINFO.band{ 1}     = {[   0     4] 'Delta'   'LFP', 0};
%      iINFO.band{ 2}     = {[   4     8] 'Theta'   'LFP', 0};
%      iINFO.band{ 3}     = {[   4     8] 'ThetaR'  'LFP', 2};
%      iINFO.band{ 4}     = {[   8    14] 'Alpha'   'LFP', 3};
%      iINFO.band{ 5}     = {[  14    24] 'Beta'    'LFP', 4};
%      iINFO.band{ 6}     = {[  24    90] 'Gamma'   'LFP', 30};
%      iINFO.band{ 7}     = {[   0    90] 'LFP'     'LFP', 0};
%      iINFO.band{ 8}     = {[   0    90] 'LFPR'    'LFP', 30};
%      iINFO.band{ 9}     = {[  40   130] 'LFPN'    'LFP', 30};
%      iINFO.band{10}     = {[ 400  3000] 'MUA'     'MUA', 30};
%
%      iINFO.lcutoff      = 500;      % Before split. LFP region (avoid singularities)
%      iINFO.mcutoff      = 100;      % Before split. MUA region (avoid singularities)
%      iINFO.NewFs        = 250;      % All signals will be resampled at 250Hz
%      iINFO.NewFsTr      = iINFO.NewFs*0.08;
%
%    PARAMETERS FOR THE FIR FILTER
%      iINFO.flttype      = 'cheby2';
%      iINFO.lstop        = 1;
%      iINFO.mstop        = 50;
%      iINFO.hstop        = 50;
%      iINFO.dB           = 60;
%      iINFO.passripple   = 0.1;
%      iINFO.NewFsTr      = 40;       % 60dB decay width for above resampling
%
...
%
%    SDU conversion
%      iINFO.conv2sdu   = 1;       1:tosdu,2:detrend,3:zerobase, can be char
%
%    If memory problem
%      iINFO.save_memory = 1;
%
%    Despiking to avoid effects of spikes on LFP
%      ANAP.sigetblp.despike      = 1;
%
%  EXAMPLE :
%    % this is just tesing, use expgetblp().
...
%
%  VERSION :
%    1.00 29.07.04 NKL first release.
%    1.01 31.08.04 YM  improved memory usage since Matlab7 seems to have memory leaks.
%    1.02 22.12.04 YM  use decimate() for LFP to make the same time-length of MUA/LFP
%    1.03 14.02.05 YM  supports FIR filters
%    1.04 24.11.05 YM  supports the exact way extracting bands developed by Andrei/Arthur.
%    1.05 25.11.05 YM  returns filter parameters as oSig.filters.
%    1.06 20.12.05 YM  supports mirroring signal, conv2sdu can be char.
%    1.07 20.12.05 YM  no use of Hilbert transform suggested by Arthur.
%    1.08 15.01.06 YM  supports iINFO for sesgetblpFIX
%    1.10 13.02.06 YM  do MUA bands first to avoid memory problems of d01nm5,exp=9
%    1.11 15.05.09 YM  add missing 40-60Hz band as lgamma.
%    1.12 15.02.13 YM  supports despiking.
%    1.20 23.05.13 YM  no longer use "anap" in the session file, see expgetblp().
%    1.21 13.06.13 YM  use [NCh 1:NCh-1] for loops, but very little improvement.
%    1.22 28.02.14 YM  supports 'detrend' before normalization.
%    1.23 22.02.16 YM  errors when filter-len > data-len.
%
% See also SESGETBLP EXPGETBLP BLPFREQZ RESAMPLE FILTFILT HILBERT clndespike
...
info.lBands    = [];  % Bands in the LFP range, updated later.
info.mBands    = [];  % Bands in the MUA range, updated later.
%info.lBands    = [1:7];  % Bands in the LFP range 
%info.mBands    = [8];    % Bands in the LFP range

info.lcutoff    = 800;      % Before splitting the LFP region (to avoid singularities)
info.mcutoff    = 100;      % Before splitting the MUA region (to avoid singularities)
info.NewFs      = 800;      % All signals will be resampled at 250Hz
...
% LowFs/LowFsTr are only For the Andrei Functions...
...
% UPDATE .lBands/.mBands by .band
info.lBands = zeros(1,length(info.band));
info.mBands = zeros(1,length(info.band));
for N = 1:length(info.band),
  if strcmpi(info.band{N}{3},'LFP'),
    info.lBands(N) = 1;  
  else
    info.mBands(N) = 1;  % Bands in the MUA range
  end
end
info.lBands = find(info.lBands > 0);  % Bands in the LFP range
info.mBands = find(info.mBands > 0);  % Bands in the MUA range

% validate info.decimate
decHz = 1/(Sig.dx(1)*info.decimate);
lfpHz = 0;
for N = 1:length(info.band)
  if strcmpi(info.band{N}{3},'LFP'),
    lfpHz = max(lfpHz,info.band{N}{1}(end));
  end
end
if decHz < lfpHz*2,
  fprintf(' info.deciate=%d-->',info.decimate);
  info.decimate = floor((1/Sig.dx(1))/(lfpHz*2*1.25));
  fprintf('%d', info.decimate);
end
clear decHz lfpHz;
...
fprintf('\n %s:BAND=%d',mfilename,length(info.lBands)+length(info.mBands));
for N = 1:length(info.lBands),
  tmpband = info.band{info.lBands(N)};
  fprintf(' %s[%g-%g/%g]',tmpband{2},tmpband{1}(1),tmpband{1}(2),tmpband{4});
end
for N = 1:length(info.mBands),
  tmpband = info.band{info.mBands(N)};
  fprintf(' %s[%g-%g/%g]',tmpband{2},tmpband{1}(1),tmpband{1}(2),tmpband{4});
end
clear N tmpband;

fprintf('\n           FIR=%d MIRROR=%d', USE_FIR, info.mirror);
...
if any(info.despike) && ~strcmpi(info.despike,'none'),
  Spkt = sigload(Cln.session,Cln.ExpNo(1),'Spkt');
  if isempty(Spkt),
    sesgetspk(Cln.session,Cln.ExpNo(1));
    Spkt = sigload(Cln.session,Cln.ExpNo(1),'Spkt');
  end
  if length(info.despike) > 1,
    SPKWIN_MS = info.despike;
  else
    SPKWIN_MS = [-1 2];
  end
  fprintf('despike([%g %g]ms,',SPKWIN_MS(1),SPKWIN_MS(2));
  if any(strcmpi(info.despike,{'select' 'selected'}))
    if ~isfield(Spkt,'times_spkcdt'),
      Spkt = spkselect(Spkt);
    end
    spktimes = Spkt.times;
    fprintf('selected-spikes).');
  else
    if isfield(Spkt,'times_spkcdt'),
      spktimes = Spkt.times_spkcdt;
    else
      spktimes = Spkt.times;
    end
    fprintf('all spike-candidates).');
  end
  tSig = clndespike(Cln,spktimes,'spkwin',SPKWIN_MS,'SpkDt',Spkt.dt,'verbose',0);
  clear Spkt SPKWIN_MS spktimes;
  % Resample before 1Hz cutoff - highly reduce computation time
  tSig = resampSig(tSig,info.NewFs,info.NewFsTr,info);
else
  % Resample before 1Hz cutoff - highly reduce computation time
...
for B=info.mBands,
  band = info.band{B};
  fprintf('BP[%g-%g].',band{1}(1),band{1}(2));
...
oSig.info       = info;
oSig.info.date  = date;
oSig.info.time  = datestr(now,'HH:MM:SS');
%oSig.info.func  = dir(which(mfilename));
oSig.filters    = FiltPrms;


% detrend before normalization
if isfield(info,'detrend') && any(info.detrend)
  tmpsz = size(oSig.dat);
  % compute DC offsets to recover after normalization
  oSig.dat = reshape(oSig.dat,[tmpsz(1) prod(tmpsz(2:end))]);
  dcoffs   = nanmean(oSig.dat,1);
  oSig.dat = reshape(oSig.dat,tmpsz);
  
  fprintf(' detrend...');
  oSig = sigdetrend(oSig);
...
if ~isempty(info.conv2sdu),
  ...
  if ~isempty(tmpmethod) && ~any(strcmpi({'none','no'},tmpmethod)),
    if isfield(info,'detrend') && any(info.detrend)
      % if detrended, then may need to recover DC offsets
      switch lower(tmpmethod)
       case { 'tosdu', 'sdu' }
        KEEP_DC = 0;
      ...
      if any(KEEP_DC)
        ...
        % recover DC offsets
        oSig.dat = reshape(oSig.dat,[tmpsz(1) prod(tmpsz(2:end))]);
        oSig.dat = bsxfun(@plus, oSig.dat ,dcoffs);
        oSig.dat = reshape(oSig.dat,tmpsz);
      end
    end
...
DECI = info.decimate;
if isfield(info,'save_memory'),
  SAVE_MEMORY = info.save_memory;
end
if ~isequal(SAVE_MEMORY,1),
  SAVE_MEMORY = 0;
  if verLessThan('matlab','7.9'),
    if numel(Sig.dat)*8 > 768e+6,  SAVE_MEMORY = 1;  end
  else
    user = memory;
    if numel(Sig.dat)*8*2 > user.MaxPossibleArrayBytes,  SAVE_MEMORY = 1;  end
    clear user;
  end
end
...
if ~isempty(info.mBands),
  ...
  fprintf('HP[%gHz].',info.mcutoff);
  mSig = DoFilter(Sig,info.mcutoff,'high',[],info,0,SAVE_MEMORY);
  if DEBUG,
    [pxx f] = pwelch(Sig.dat(:,1),1024,512,1024,1/Sig.dx);
    plot(f, pxx, 'color','r');
    hold on;
    [pxx f] = pwelch(mSig.dat(:,1),1024,512,1024,1/mSig.dx);
    plot(f, pxx, 'color','b');
    hold on;
  end;
end
...
for B=info.mBands,
  ...
  fprintf('BP[%g-%g].',band{1}(1),band{1}(2));
  if isempty(SAME_SIG),
    % ATN_NKL: This is the bandpass filtering of the signal
    if DEBUG,
      [tmp, fprms] = DoFilter(mSig,band{1},'bandpass',info.mstop,info,0,SAVE_MEMORY);
      [pxx f] = pwelch(tmp.dat(:,1),1024,512,1024,1/tmp.dx);
      plot(f, pxx, 'color','k','linewidth',2);
      keyboard
    else
      % [tmp fprms] = DoFilter(mSig,band{1},'bandpass',info.mstop,info,USE_FIR,SAVE_MEMORY);
      % !!!!!!!!!!!!!!!!!!!!! ATTENTION
      % By accident we noticed (NKL-YM) that the FIR cuts too short the low-end of MUA We
      % also noticed by accident (with a mistake of NKL in using the USE_FIR that most of
      % the robust responses (and clustering) in MUA can be obtained when the high
      % frequencies are selected!!  To select high frequencies we have increased the MUA
      % band, to 1500 - 3400 Hz. But we also leave the left boundary decay gradually.
      [tmp, fprms] = DoFilter(mSig,band{1},'bandpass',info.mstop,info,0,SAVE_MEMORY);
   end;
  else
    fprintf('=');
...
fprintf('\n %s:LFP ',mfilename);
if any(info.despike) && ~strcmpi(info.despike,'none')
  Spkt = sigload(Sig.session,Sig.ExpNo(1),'Spkt');
  if isempty(Spkt),
    sesgetspk(Sig.session,Sig.ExpNo(1));
    Spkt = sigload(Sig.session,Sig.ExpNo(1),'Spkt');
  end
  if length(info.despike) > 1,
    SPKWIN_MS = info.despike;
  ...
  lSig = clndespike(Sig,spktimes,'spkwin',SPKWIN_MS,'SpkDt',Spkt.dt,'verbose',0);
  clear Spkt SPKWIN_MS spktimes;
  % Reduce BandWidth Before Extracting LFPs
  fprintf('LP[%gHz].',info.lcutoff);
  lSig = DoFilter(lSig,info.lcutoff,'low',[],info,0,SAVE_MEMORY);
else
  % Reduce BandWidth Before Extracting LFPs
  fprintf('LP[%gHz].',info.lcutoff);
...
fprintf('DEC[%d:%gHz].',DECI,1/lSig.dx/DECI);
lSig = DoDecimate(lSig,1/lSig.dx/DECI);
%fprintf('RESAMP[%gHz].',info.lcutoff*2);
...
for B=info.lBands,
  ...
  fprintf('BP[%g-%g].',band{1}(1),band{1}(2));
  if isempty(SAME_SIG),
    [tmp, fprms] = DoFilter(lSig,band{1},'bandpass',info.lstop,info,USE_FIR,SAVE_MEMORY);
  else
    fprintf('=');
    tmp = SAME_SIG;
  end
...
if isfield(info,'detrend') && any(info.detrend)
  ...
  % tmpz = abs(zscore(oSig.dat,[],1));
  % fprintf(' detrend2...');
  % for K = 1:size(oSig.dat,2)
  %   oSig.dat(:,K) = detrend(oSig.dat(:,K),'linear',find(tmpz(:,K) < 1.0));
  % end
...
if USE_FIR == 0,
  ...
  if max([length(b),length(a)]) > size(Sig.dat,1),
    error('\n ERROR %s.DoFilterHilbert(FIR): filter[%s]Hz:%gsec > data:%gsec\n The filter length is longer than data.  FYI, our FIR filter needs a x3.6 longer period than the specified frequency.',...
          mfilename, deblank(sprintf('%g ',lim)),max([length(b),length(a)])*Sig.dx, size(Sig.dat,1)*Sig.dx);
  end
end
...
if USE_FIR == 0,
  ...
  [b,a,fname,ftype,forder] = subDesignFIRFilter(1/Sig.dx,lim,mode,edgeFlt,info);
  if max([length(b),length(a)]) > size(Sig.dat,1),
    error('\n ERROR %s.DoFilter(FIR): filter[%s]Hz:%gsec > data:%gsec\n The filter length is longer than data.  FYI, our FIR filter needs a x3.6 longer period than the specified frequency.',...
          mfilename, deblank(sprintf('%g ',lim)),max([length(b),length(a)])*Sig.dx, size(Sig.dat,1)*Sig.dx);
  end
end
...
fprintf('BP[%g-%gHz].',info.band{B}{1}(1),info.band{B}{1}(2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Sig=sublfprestore(Sig,info)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
if isfield(info,'lfprestore') && any(info.lfprestore),
  fprintf(' lfprestore[');
  fprintf('1HzHP.');
  %Sig = subAndrei_filterSig(Sig,[1 99999],info.lstop,info);
  Sig = DoFilter(Sig,1,'high',info.lstop,info,1,0);
  %fprintf('%s testing high pass filtering',mfilename);
  %Sig = DoFilter(Cln,1,'high',[],info,0);
  Sig = subintegrate(Sig);
  fprintf(']');
end;
return;
