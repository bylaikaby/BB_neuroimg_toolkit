...
%    1.01 21.07.17 YM  supports old files which sites are not saved independently.
%
...
if isempty(FILENAME),
  ...
  if isempty(EvtSites),
    ...
    if isempty(tmpdirs),
      % it is not site-wise file (likely old-format), just load it.
      if exist(tmpfile,'file')
        try
          EVT = load(tmpfile,EvtVarName);
          EVT = EVT.(EvtVarName);
        catch
          error(' ERROR %s: ''%s'' not found in %s.\n',mfilename,EvtVarName,tmpfile);
        end
      else
        error(' ERROR %s: no file found, %s\n',mfilename,tmpfile);
      end
    else
      % OK, event-files are saved in site-wise.
