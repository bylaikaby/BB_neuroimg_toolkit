...
% YM  28.06.11 supports "stimch" if exists.
...
if ~any(strcmpi(SigName,{'tcImg','roiTs','froiTs','troiTs','tfroiTs','Cln','blp','mFid',...
                      'nClnblp','fClnblp','mClnblp','eeg','model','ClnSpc','Spkt','Sdf'})),
...
switch lower(SigName),
 case {'blp','nclnblp','fclnblp','mclnblp'}
  if DIM == 5,
    % GROUPED BLP
...
% if oSig has .stimch
if isfield(Sig{1},'stimch'),
  oSig.stimch = [];
  for T = 1:length(Sig),
    oSig.stimch = sub_cat_stimch(oSig.stimch,Sig{T}.stimch);
  end
end
...
% ===============================================
% cat additional analog data
function STIMCH = sub_cat_stimch(STIMCH,iSTIMCH)
% ===============================================
if iscell(iSTIMCH),
  if isempty(STIMCH),  STIMCH = cell(1,length(iSTIMCH));  end
  for N = 1:length(iSTIMCH),
    STIMCH{N} = sub_cat_stimch(STIMCH{N},iSTIMCH{N});
  end
  return;
end

iSTIMCH.dat = nanmean(iSTIMCH.dat,3);

if isempty(STIMCH),
  STIMCH = iSTIMCH;
else
  STIMCH.dat = cat(1,STIMCH.dat,nanmean(iSTIMCH.dat,3));
  if isfield(STIMCH,'sigsort') && isfield(STIMCH.sigsort,'nrepeats'),
    STIMCH.sigsort.nrepeats = cat(2,STIMCH.sigsort.nrepeats(:)',iSTIMCH.sigsort.nrepeats(:)');
  end
end
