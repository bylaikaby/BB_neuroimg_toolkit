function oSig = sigspc(iSig, WT, dT, NFFT, WinFct,SPC_METHOD)
%SIGSPC - Make spectrogram from signal iSig
%	USE: oSig = SIGSPC(iSig, WT, dT, NFFT, WinFct, SPC_METHOD)
...
%   YM,  14.09.11  supports 'SPC_METHOD' as 'pmtm','spectrogram' or 'specgram'.
%
%%% TODO: Smoothing kernel?

if nargin < 1,  help sigspc;  return;  end;
...
% if ~exist('SPC_METHOD','var'),  SPC_METHOD = 'pmtm';  end
if ~exist('SPC_METHOD','var'),  SPC_METHOD = 'specgram';  end
...
if PRINT_INFO,
  fprintf(' sigspc %s [NFFT:%d(%.2fs), WINDOW:%s/%d(%.2fs), NSHIFT:%d(%.2fs), dF:%6.3fHz]\n',...
        SPC_METHOD,...
        NFFT, NFFT*iSig.dx,...
        WinFct,N,N*iSig.dx,...
  ...
  for ObspNo = NoObsp:-1:1,
    switch lower(SPC_METHOD)
     case {'pmtmspectrogram' 'pmtm'}
      ...
      NFFT = length(WINDOW);
      %%%%%%%%%%%%%%%%%%%%%%%%  ATTENTION!!!!!!!!!!!!!!!!1
      [tmpSpc,F,T] = pmtmspectrogram(iSig.dat(:,ChanNo,ObspNo),WINDOW,NOVERLAP,NFFT,1/Ts);
      tmpSpc = tmpSpc .* Ts;
     case {'spectrogram'}
      [tmpSpc,F,T] = spectrogram(iSig.dat(:,ChanNo,ObspNo),WINDOW,NOVERLAP,NFFT,1/Ts);
      if POWER,
