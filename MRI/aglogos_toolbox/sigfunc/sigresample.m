...
for N = 1:2:length(varargin)
  switch lower(varargin{N}),
   ...
   otherwise,
    fprintf('SIGRESAMPLE: Unknown varargin\n'); keyboard;
  end
end

if NEWDX==Sig.dx & isempty(DatField), oSig = Sig;  return; end;

if ~isempty(DatField) & isfield(Sig,'dxorg'),
  % fprintf('Resampling FIELD "%s"\n', DatField);
  oSig = Sig;
  sig = Sig;
  sig.dat = Sig.(DatField);
  sig.dx = sig.dxorg;
  sig = sigresample(sig, NEWDX);
  oSig.(DatField) = sig.dat;
  return;
end;
