...
if nargin < 3,
  ...
  for N = 1:length(SigName),
    if iscell(Sig.(SigName{N})),
      ...
      if ~isfield(Sig.(SigName{N}),'dir'),
        continue;
      end;
      Sig.(SigName{N}).grp = grp;
      if isfield(pars,'evt') & ~isempty(pars.evt) & ~isfield(Sig.(SigName{N}),'sigsort'),
        Sig.(SigName{N}).evt = pars.evt;
      end
      if isfield(pars,'stm') & ~isempty(pars.stm) & ~isfield(Sig.(SigName{N}),'sigsort'),
        if ischar(SigName{N}),
          if ~strcmp(SigName{N}(1:2),'es'),
            Sig.(SigName{N}).stm = pars.stm;
          end;
        else
          if ~strcmp(SigName{N}{1}(1:2),'es'),
            Sig.(SigName{N}{1}).stm = pars.stm;
          end;
        end;
    ...
    if strcmpi(SigName{N},'tcImg') | strcmpi(SigName{N},grp.name),
      Sig.(SigName{N}).usr.pvpar = pars.pvpar;
    end
    if nargout,
      varargout{N} = Sig.(SigName{N});
    else
      assignin('caller',SigName{N},Sig.(SigName{N}));
    end;
  end
  return;
end
  ...
  for N = 1:length(SigName),
    switch lower(SigName{N}),
     case { 'cln','clnspc','tcimg'}
      ...
      Sig{N} = subsigload(catfilename(Ses,ExpNo,SigName{N}), SigName{N}, Ses, ExpNo);
     case { 'spktblp' ,'spktcln','brsttblp','brsttcln',...
            'atspktblp' ,'atspktcln','atbrsttblp','atbrsttcln' }
      Sig{N} = subsigload(catfilename(Ses,ExpNo,SigName{N}), SigName{N}, Ses, ExpNo);
	 case lower(Ses.ctg.GrpDEPSigs)
	  Sig{N} = subsigload(catfilename(Ses,ExpNo,'contrasts'),SigName{N}, Ses, ExpNo);
     otherwise
      Sig{N} = subsigload(catfilename(Ses,ExpNo,'mat'), SigName{N}, Ses, ExpNo);
    end
  end
end
...
for N = 1:length(Sig),
  ...
  if isstruct(Sig{N}),
    if strcmp(Sig{N}.dir.dname(1:2),'es'), continue; end;
  elseif isstruct(Sig{N}{1}),
    if strcmp(Sig{N}{1}.dir.dname(1:2),'es'), continue; end;
  end;
  ...
  if iscell(Sig{N}),
    ...
    if isfield(pars,'stm') & ~isempty(pars.stm) & ~isfield(Sig{N},'sigsort'),
      Sig{N}.stm = pars.stm;
    end
