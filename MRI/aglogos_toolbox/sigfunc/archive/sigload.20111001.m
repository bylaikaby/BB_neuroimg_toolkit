...
for N = 1:length(SigName),
  [tmpsig tmpname] = sub_sigload(Ses,ExpNo,SigName{N});
  if nargout,
    varargout{N} = tmpsig;
  else
...
function [Sig SigName] = sub_sigload(Ses,ExpNo,SigName)
% ----------------------------------------------------------------
...
if ischar(ExpNo) || isstruct(ExpNo),
  ...
  if strcmpi(SigName,'tcImg'),
    Sig = sub_load('tcImg.mat', grp.name);
  else
    Sig = sub_load(sprintf('%s.mat',grp.name), SigName);
  end
else
  ...
  Sig = sub_load(matfile, tmpsigname, Ses, ExpNo);
  SigName = tmpsigname;
end
...
if iscell(Sig),
  ...
  Sig.grp = grp;
  if isfield(pars,'evt') && ~isempty(pars.evt) && ~isfield(Sig,'sigsort'),
    Sig.evt = pars.evt;
  end
...
% ===================================================================
function Sig = sub_load(Filename,SigName, Ses, ExpNo)
