...
for iExp = 1:length(EXPS),
  ...
  switch SigName,
      ...
      for RoiNo = 1:length(oSig),
        if iscell(oSig{RoiNo}),
          for T = 1:length(oSig{RoiNo}),
            oSig{RoiNo}{T}.ExpNo = grp.exps;
            for M = 1:length(oSig{RoiNo}{T}.r),
              tmpidx = (oSig{RoiNo}{T}.r{M} > Rthr) & (oSig{RoiNo}{T}.p{M} < Pthr);
              ORSEL{RoiNo}{T}{M} = tmpidx;
            end
          end;
        else
          oSig{RoiNo}.ExpNo = grp.exps;
          for M = 1:length(oSig{RoiNo}.r),
   ...
   case {'roiTs'}
    ...
    if ~isfield(Sig{1},'r') | ~isfield(Sig{1},'p'),
      if ~isempty(RoiNames), Sig = mroitsget(Sig,[],RoiNames); end
      if iExp == 1,
...
if RECOVER_STRUCT > 0 & length(oSig) == 1,
  oSig = oSig{1};
end
