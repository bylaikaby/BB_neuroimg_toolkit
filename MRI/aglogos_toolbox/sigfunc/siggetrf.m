function oSig = siggetrf(iSig,ithreshold,offsetT,NoAvg,method,ADJUST_THRESHOLD,FLT_PARS)
...
%  0.92 20.12.07  YM,  supports ADJUST_THRESHOLD to control thresholding
%
...
if nargin < 4,  NoAvg = [];  end;
if nargin < 5,  method = []; end;

if ~exist('ADJUST_THRESHOLD','var'),  ADJUST_THRESHOLD = 1;  end
if isempty(NoAvg), NoAvg = 1500;  end
if isempty(method),  method = 'range';  end
...
oSig = iSig;
if isfield(oSig,'tosdu'),
  oSig = rmfield(oSig,{'tosdu'});
end;
...
for chan = size(iSig.dat,2):-1:1,
  ...
  switch lower(method),
   case { 'range' }
	...
	elseif ADJUST_THRESHOLD > 0 & length(rIndex)<NoAvg,
