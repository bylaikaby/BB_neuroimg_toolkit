function varargout = sigsort(iSig,sortPar,PreT,PostT,CHECK_JAWPO,CHECK_CENTROID,Toffset,DoDetrend)
...
%  NOTES   :
%  VERSION
%    0.90 04.02.04 YM   first release
...
%    1.03 17.10.06 YM   supports Toffset.
%    1.04 19.11.08 YM   supports detrend.
%    1.05 07.02.11 YM   use floor(round(*1000)/1000) to avoid approximation error.
%    1.06 28.02.11 YM   make sure PreT/PostT as positive value.
%    1.07 20.06.11 YM   supports ".stimch" also.
%    1.08 31.03.20 YM   clean-up.
%
...
if ~exist('CHECK_CENTROID','var') || isempty(CHECK_CENTROID),  CHECK_CENTROID = 0;  end
if ~exist('Toffset','var') || isempty(Toffset),  Toffset = 0;  end
if ~exist('DoDetrend','var') || isempty(DoDetrend),  DoDetrend = 0;  end
...
if iscell(iSig)
  for N = 1:length(iSig)
    oSig{N} = subDoSort(iSig{N},sortPar,PreT,PostT,CHECK_JAWPO,CHECK_CENTROID,Toffset,DoDetrend);
  end
else
  oSig = subDoSort(iSig,sortPar,PreT,PostT,CHECK_JAWPO,CHECK_CENTROID,Toffset,DoDetrend);
end
...
function oSig = subDoSort(iSig,sortPar,PreT,PostT,CHECK_JAWPO,CHECK_CENTROID,Toffset,DoDetrend)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for N = 1:length(sortPar.label)
  ...
  for K = 1:length(TONSET),  TONSET{K} = TONSET{K} + Toffset;  end
  ...
  % 07.06.06 YM  checks centroid of tcImg
  if CHECK_CENTROID && isfield(iSig,'session') && isawake(iSig.session,iSig.ExpNo(1))
    [TONSET, OBS] = subValidateCentroid(iSig,OBS,TONSET,PRE_PTS,POSTLEN);
  end

  oSig{N} = tSig;
  switch datname
   case {'blp' 'cblp' 'nClnblp' 'fClnblp' 'mClnblp'}
    % blp.dat(t,chan,band)
  ...
  if isfield(iSig,'stimch')
    tmppreT  = PRE_PTS*iSig.dx(1);
    tmppostT = POSTLEN*iSig.dx(1);
    oSig{N}.stimch = sub_stimch(iSig.stimch,datfile,OBS,TONSET,tmppreT,tmppostT);
  end
  if isfield(iSig,'movie')
    tmppreT  = PRE_PTS*iSig.dx(1);
    tmppostT = POSTLEN*iSig.dx(1);
    oSig{N}.movie = sub_stimch(iSig.movie,  datfile,OBS,TONSET,tmppreT,tmppostT);
  end
  ...
  if ~isempty(sortPar.tonset{N})
    oSig{N}.stm.time   = {sortPar.tonset{N}{1} - sortPar.tonset{N}{1}(1) + PreT};
  else
    oSig{N}.stm.time = {[]};
  end
  oSig{N}.stm.tvol   = {STMDT/sortPar.imgtr};
  ...
  if isfield(oSig{N},'evt') && ~isempty(oSig{N}.evt)
    obs = [];
    obs.adflen  = 0;
    obs.beginE  = 0;
    obs.endE    = (PRE_PTS+POSTLEN)*iSig.dx*1000;
    obs.mri1E   = [];
    obs.trialE  = [];
    obs.fixE    = [];
    obs.t       = [];
    ...
    obs.trialID = [];
    obs.trialCorrect = [];
    obs.times.begin = obs.beginE;
    obs.times.end   = obs.endE;
    obs.times.ttype = [];
    obs.times.stm   = oSig{N}.stm.time{1} * 1000;
    obs.params.prm     = [];
    obs.params.stmid   = oSig{N}.stm.v{1};
    obs.params.trialid = [];
    obs.params.stmdur  = oSig{N}.stm.tvol;
    obs.origtimes = [];
    obs.eye = [];
    obs.jawpo = [];
    obs.status = 1;
    if isfield(sortPar,'prmnames') && ~isempty(sortPar.prmnames),
      ...
      obs.trialCorrect = [1];
      obs.times.ttype = 0;
      obs.params.trialid = N-1;
      obs.params.trialCorrect = [1];
    end
    oSig{N}.evt.obs = {obs};
  end
...
if DoDetrend
  for N = 1:length(oSig)
    switch datname
     case {'tcImg'}
      % tcImg.dat(x,y,slice,t)
      tmpsz = size(oSig{N}.dat);
      oSig{N}.dat = reshape(oSig{N}.dat,[prod(tmpsz(1:3)) tmpsz(4)]);
      tmpdc = nanmean(oSig{N}.dat,2);
      for K = 1:size(oSig{N}.dat,1)
        oSig{N}.dat(K,:) = detrend(oSig{N}.dat(K,:)) + tmpdc(K);
      end
      oSig{N}.dat = reshape(oSig{N}.dat,tmpsz);
     otherwise
      tmpsz = size(oSig{N}.dat);
      oSig{N}.dat = reshape(oSig{N}.dat,[tmpsz(1) prod(tmpsz(2:end))]);
      tmpdc = nanmean(oSig{N}.dat,1);
      oSig{N}.dat = detrend(oSig{N}.dat);
      for K = 1:size(oSig{N}.dat,2)
        oSig{N}.dat(:,K) = oSig{N}.dat(:,K) - tmpdc(K);
      end
      oSig{N}.dat = reshape(oSig{N}.dat,tmpsz);
    end
  end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [TONSET, OBS] = subValidateCentroid(iSig,OBS,TONSET,PRE,POST)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~isfield(iSig,'centroid') || isempty(iSig.centroid),  return;  end
if ~isfield(iSig,'ds') || isempty(iSig.ds), return;  end
...
fprintf('cent%d/%d ',length(valididx),n);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function oSig = sub_stimch(iSig,datfile,OBS,TONSET,PreT,PostT)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if iscell(iSig)
  for N = 1:length(iSig)
    oSig{N} = sub_stimch(iSig{N},datfile,OBS,TONSET,PreT,PostT);
  end
  return
end

PRE_PTS = round(PreT/iSig.dx(1));    % in data pts
POSTLEN = round(PostT/iSig.dx(1));   % in data pts

oSig = iSig;
oSig.dat = [];
oSig.dat = subSortSig(iSig,datfile,OBS,TONSET,PRE_PTS,POSTLEN);
oSig.(mfilename).nrepeats = length(OBS);
oSig.(mfilename).PreT     = PreT;
oSig.(mfilename).PostT    = PostT;
oSig.(mfilename).pre_pts  = PRE_PTS;
oSig.(mfilename).len_pts  = PRE_PTS+POSTLEN;
...
for N = length(OBS):-1:1
  sel = selidx + floor(round(TONSET{N}(1)/iSig.dx(1)*1000)/1000);
  if sel(1) <= 0
    idx1 = find(sel <= 0);
    idx2 = find(sel >  0);
    %tmpdat(:,idx1,:) = NaN;
...
for N = length(OBS):-1:1
  ...
  if sel(1) <= 0
    idx1 = find(sel <= 0);
    idx2 = find(sel >  0);
    tmpdat(idx1,:,:) = nanmean(iSig.dat(idx1,:,:,:),4);
    tmpdat(idx2,:,:) = iSig.dat(sel(idx2),:,:);
  elseif sel(end) > maxlen
    %keyboard
    idx1 = find(sel <= maxlen);
    idx2 = find(sel >  maxlen);
    tmpdat(idx1,:,:) = iSig.dat(sel(idx1),:,:);
    tmpdat(idx2,:,:) = nanmean(iSig.dat(idx2,:,:,:),4);
  else
    tmpdat = iSig.dat(sel,:,:);
  end
  oSigDat(:,:,:,N) = tmpdat;
