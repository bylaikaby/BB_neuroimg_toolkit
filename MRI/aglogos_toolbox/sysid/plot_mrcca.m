function plot_mrcca(RES,varargin)
%PLOT_MRCCA - Plot tkCCA result
%  PLOT_MRCCA(RES,...) plots tkCCA results.
%
%  NOTE :
%    Lags by expmrcca/exptcor are flipped for easy comparison with HRF.
%
%  EXAMPLE :
%    res = expmrcca('e10ha1',6,'roi',{'HP' 'hele'});
%    plot_cca(res)
% 
%  VERSION : 
%    0.90 09.01.13 YM  pre-release
% 
%  See also sesmrcca expmrcca expmrcca_cv
...
for N = 1:length(RES.model),
  RES.model(N).weights = nanmean(RES.model(N).weights,3);
  RES.model(N).xcorr   = nanmean(RES.model(N).xcorr,  2);
end
...
mfigure([680 100 750 900]);
if isfield(RES,'session') && ischar(RES.session) && ~isempty(RES.session),
  set(gcf,'Name',sprintf('%s-mdl %s %s (nexp=%d)',algorithm,RES.session,RES.grpname,length(RES.exps)));
else
  set(gcf,'Name','%s-mdl',algorithm);
end
subPlotMDL(RES.model,RES.opts.algorithm,OPTS);
return
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function subPlotMDL(RES,method,OPTS)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for N = 1:length(RES),
  ...
  title(sprintf('%s Weights',RES(N).name));
  ...
  xlabel('Lags in seconds (model to BOLD)');
  ylabel('CC');
  title(sprintf('%s XCorr',RES(N).name));
end
return
