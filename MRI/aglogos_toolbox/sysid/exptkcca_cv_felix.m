...
if numel(RoiName)>1 | numel(NeuChans)>1, 
	error('Only one electrode/ROI at once');
end
...
for N = 1:length(EXPS),
  ...
  % restrict ROI to certain radius around electrode
  radius = 3;
  % the spatial resolution
  scale = diag(roits.ds);
  elepos = blp.coords;
  % the distance to the electrode
  dist = sqrt(sum(((repmat(elepos,size(roits.coords,1),1)-roits.coords) * scale).^2,2));
  exclude = dist > radius;
  roits.coords(exclude,:) = [];
  roits.dat(:,exclude) = [];
...
ff = parzenwin(5);
for i=1:numel(NEU)
%	NEU{i} = mean(cat(3,NEU{i}(1:10,:),NEU{i}(11:20,:),NEU{i}(21:30,:)),3);
%	NEU{i}([1 10],:) = [];
	NEU{i} = NEU{i} - repmat(mean(NEU{i}),size(NEU{i},1),1);
%	NEU{i} = filtfilt(ff,1,NEU{i}')';
