...
for N = 1:length(RES.ephys),
  ...
  % ?????????????
%  RES.ephys(N).xcorr   = nanmean(RES.ephys(N).weights([3:10],:),  1);
...
subPlotNEU(RES.ephys,RES.opts.algorithm);

return



function subPlotMRI(RES)
GAMMA_VAL = 1;
...
function subPlotNEU(RES,method)
