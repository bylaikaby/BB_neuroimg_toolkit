...
switch (RoiName),
 case 'Metencephalon',
  RoiName = anap.Metencephalon;
 case 'Mesencephalon',
  RoiName = anap.Mesencephalon;
 case 'Diencephalon',
  RoiName = anap.Diencephalon;
 case 'Limbic';
  RoiName = anap.Limbic;
 case 'Neocortex';
  RoiName = anap.Neocortex;
end;
  
if isimaging(grp),
  if VERBOSE, fprintf(' roits.');  end

  % MriSig can be roiTs or troiTs or any other MR-TS !!!!!!!!!!!!!!!!!!!!!!!
...
if isrecording(grp),
  ...
  if strcmpi(blp.info.band{1}{2},'cln') && strcmpi(blp.info.band{1}{3},'LFP') && blp.info.band{1}{4} == 0,
    blp.dat(:,:,1) = abs(blp.dat(:,:,1));
  end
  % ??????????????????????????????????????????????????????????????????????
  % ??????????????????????????????????????????????????????????????????????
  % ??????????????????????????????????????????????????????????????????????
...
if any(strcmpi(EleName,'all')) || isempty(EleName),
  ...
  EleLabel = grp.namech;
elseif strcmp(EleName,'allgrouped'),
  ...
  blp.dat = tmp;
elseif strcmp(EleName,'hipgrouped'),
