function [RES V1 V2] = run_tcca(TYPE)
%RUN_TCCA - Run/Plot TKCCA
%  EXAMPLE :
%    [ALL V1 V2] = run_tcca('polar')
%    [ALL V1 V2] = run_tcca('spont')
%
...
RES = {};
for N = 1:length(SES),
  tmpres = sestkcca(SES{N}{1},SES{N}{2}{1},'plot',1,...
...
if ~isempty(V1),
  figure; set(gcf,'Name',sprintf('CCA-neu V1 (%s)',TYPE));
  sub_plotneu(V1);
end
if ~isempty(V2),
  figure; set(gcf,'Name',sprintf('CCA-neu V2 (%s)',TYPE));
  sub_plotneu(V2);
end
...
tmpidx = [];
for N = 1:size(newres.band),
  if strcmpi(newres.band{N}{2},'gamma'),
    tmpidx = N;  break;
  end
end
...
tmpt = -tmpt;  % flip time to match with HRF
...
title(sprintf('normalized weights (n=%d)',size(RES.weights,3)));
...
title(sprintf('xcorr with BOLD (n=%d)',size(RES.xcorr,3)));
xlabel('lags in sec (neuro to BOLD)');
ylabel('corr. coef.');
